...
Now assume that for every $z$, $X_z$ is given by a subset $S_z$ of the argument terms~$\mathcal{T}$ s.t. \[X_z=\{ f(I(\subst{t}{x}{z}))\mid f(t)\in S_z\}\]
Then the 
clash condition becomes   
    \[
      \forall z\;.\; \bigwedge_{f(t)\in \mathcal{S}_z}\bigwedge_{a\in args(F,f) } a\neq t[x\leftarrow z] 
  \]
and the propagation condition
\[
  \forall z', z\;.\; z'\prec z \Rightarrow \bigwedge_{f(t')\in \mathcal{T}, f(t)\in S_z} t'[x\leftarrow z'] \neq t[x\leftarrow z]
\]
...
\[\forall x \forall \bar{u} \;.\; congu\Rightarrow \exists \bar{v}.\ congv\wedge Q' \]
where congu is
...
and where $\bar{u}$ corresponds to the variables $u_{f(t)}$ with $f(t)\in\mathcal{T}\setminus\mathcal{S}$ and $\bar{v}$ to the variables $v_{f(t)}$ with $f(t)\in \mathcal{S}$, and $Q'$ is obtained from $Q$ by simultaneously replacing
        \begin{itemize}
...
\item 
    ...
    \mathcal{S} & \text{if } z > b_{\max}, \\[4pt]
    \mathcal{S}' & \text{if } z < b_{\min}.
    \end{cases}
  \]
\end{itemize}


Plugging this into the constraints above, we get
