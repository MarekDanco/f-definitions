...
$g^\gmodel(2), g^\gmodel(4), g^\gmodel(6), \dots$ by unifying $x$ with $0, 2, 4, \dots$ and then set $x+2 \prec x$ and define
$g^\gmodel(-2), g^\gmodel(-4), g^\gmodel(-6), \dots$ by unifying $x+2$ with $0, -2, -4, \dots$ That is, we can define
...
Idea: View quantifiers as something that expresses infinitely many constraints on entities. The problem is that a
...
\begin{definition}
    ...
    \begin{itemize}
        \item an order\footnote{For now, I assume this order to be total.} on all instantiations of quantified
              sub-formulas of $\phi$, viewing the quantifier-free part as ONE quantified formula with an empty tuple of quantified
              variables, hence having one instantiation, and
