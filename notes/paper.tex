...
\begin{definition}\todoSR{Sorry, I changed this once more. The intuition stays the same.}
...
\begin{property}
    \label{prop:cert}
    Moreover, assume a non-empty integer interval $I$ where \todoSN{$\abs{I}$ requirement added.} $\abs{I}\geq c_{l_{i^*}}^{f_{i^*}}-c_1^{f_{i^*}}$, and an entity interpretation $\alpha$ s.t.
...
\begin{proof} \todoSR{I still need to check some details, e.g., the case $r<2$.}
    ...
    \begin{description}
        ...
        \item[Case $x < \min I$:] The existential entity~$f_{i^*}(x+c^{f_{i^{*}}}_1)$, is neither allowed to be assigned a value by $\alpha$, nor allowed to occur in any instantiation~$x'$ with $\max I<x'$, nor by any $x'$ with $x<x'<\min I$. The first condition is ensured by hypothesis (\textit{iii}), the second condition \todoSR{This is the ``glueing together'' mentioned by Mikoláš below.} is followed from the lowest argument $(\max I+1) + c^{f_{i^*}}_1 \geq (\min I+\Delta) + c^{f_{i^*}}_1 + 1=\min I+c^{f_{i^*}}_{l_{i^*}}+1>(\min I-1)+c^{f_{i^*}}_{l_{i^*}}\geq x+c^{f_{i^{*}}}_1$, and the third condition by the fact that $x'+c^{f_{i^{*}}}_1$ is the lowest argument of $f_{i^*}$.
    \end{description}
...
when going in the increasing (decreasing) direction, the same disjunct contains the
maximal (minimal) offset\mj{Why?}.
As a result, the algorithm effectively tries to satisfy the formula using
only this single disjunct, which isn't sound\mj{not sure that's the right word}. Therefore, a different approach is required for handling disjunctions.
...
\begin{definition}[Cell]
    A \emph{cell} is a term of the form $f(n)$ where $f$ is an uninterpreted function symbol
    and $n\in\mathbb{Z}$.\mj{difference from \cref{def:entity}?}
\end{definition}

% \begin{definition}[Maximal Cell]
% 	The \emph{maximal cell} of $f$ in a formula $\varphi$ is the cell $f(t)$ where $t$ is the maximal
...
Suppose we are given a formula of the form $\forall x. \,l_1(x) \lor \dots \lor l_n(x)$.
For each disjunct we can form a precondition formula on $x$ in a way that
if the precondition holds, that literal has to be satisfied.
More precisely, if we construct formulas $\mathit{pre}_1, \dots, \mathit{pre}_n$ so that
\mj{What can the precondition comprise?, e.g., is $c|x$ allowed?}
\marek{It is very difficult for me to argue why these conditions are sufficient. I don't think they are.
    I also think this is a significantly more difficult problem than the original one,
    since we assume none of the disjuncts satisfy \req. I will take a look at this later.}
\begin{enumerate}
    \item\label{pre:1} $\forall x. \,\mathit{pre}_i(x) \implies l_i(x)$ is satisfiable for all $i$,
    \item\label{pre:2} $\forall x. \,\bigvee_{i = 1}^n \mathit{pre}_i(x)$ is valid,
    \item\label{pre:3} there are no $i\not=j, x_1, x_2$ such that $\mathit{pre}_i(x_1) \land \mathit{pre}_j(x_2)$
          and $l_i[x \mapsto x_1]$, $l_j[x \mapsto x_2]$ write to the same cell in the same direction,
\end{enumerate}
then we can solve instead the following formula\mj{Why do we need to still solve it isn't it clearly satisfiable?}
...
we try to write to $f(6)$ two times with different values.\mj{Further, the is
