...
\begin{definition}[Satisfiability Certificate]
    ...
    \begin{itemize}
        \item an order\footnote{For now, I assume this order to be total.} on all instantiations
              \todo{Do you mean an order on the constraints these instantiations
                  produce? -Marek}\todoSR{I used the term ``constraint'' only to
                  express (my own) intuition. I also had the feeling that for a
                  formal definition via ``instantiation of quantifier'' may be more
                  straightforward. None of this is set in stone, of cource.}
              of quantified
