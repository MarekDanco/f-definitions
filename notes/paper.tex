...
\begin{definition}[Entity]
    An \emph{entity} of a formula is either a numerical variable or a pair consisting of an  uninterpreted
    function symbol and an integer (which models an index of the uninterpreted function symbol).
\end{definition}
...
\begin{definition}[Satisfiability Certificate]
    ...
    \begin{itemize}
        \item an order\footnote{For now, I assume this order to be total.} on all instantiations
              \todo{Do you mean an order on the constraints these instantiations produce? -Marek}\todoSR{I used the term ``constraint'' only to express (my own) intuition. I also had the feeling that for a formal definition via ``instantiation of quantifier'' may be more straightforward. None of this is set in stone, of cource.} of quantified
              sub-formulas of $\phi$, viewing the quantifier-free part as ONE quantified formula with
              an empty tuple of quantified variables, hence having one instantiation, and
        \item for each instantiation of a quantified formula
              \begin{itemize}
                  \item a subset of its entities marked as existential such that the existential entities do not occur in any
                        formula instantiation occurring before according to the given order, and
                  \item a certificate showing that for all values of the non-existential entities there exist values for
                        all other entities satisfying this instantiation, which we will call \emph{satisfiability propagator}.
              \end{itemize}
    \end{itemize}
\end{definition}

The satisfiability propagators typical have the form entity=term, where the left-hand sides are the existential
variables\footnote{At some point, we might allow satisfiability propagators of a different form, for example,
