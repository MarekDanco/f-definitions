...
\begin{definition}[entity interpretation]
	An \emph{entity interpretation} is a partial function whose domain is the set of entities.\todoSR{changed to take Mikoláš's comments into account}
\end{definition}
...
For this, let $inst(u)$ either be the  instantiation $\psi$ of a quantified sub-formula with $u\in\Gamma_I(\psi)$ and where $u$ is existential, or $\bot$ if there is no such instantiation\marek{How can this happen?}\todoSR{It can happen with function symbols of arity $\geq 2$.}
% We require all arguments to be of the form $x + k$, so for every entity there is a minimal $\psi$ that contains it. From minimality this $u$ has to be existential in $\psi$. Implicitly $u$ isn't in the ground part.}.
