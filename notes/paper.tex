...
%%%%%%%%%%%
Formula~\ref{eq:completeness} satisfies \req but
...
\begin{property}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		\item For every function $f_j$ (where $1 \leq j \leq r$) and for every ground argument term $t \in \args(F, f_j)$, we have $\alpha(t) \leq \overline{I} + c^{f_j}_{l_j}$
		\item For every function $f_j$ (where $1 \leq j \leq r$) and for every ground argument term $t \in \args(F, f_j)$, we have $\underline{I} + c^{f_j}_1 \leq \alpha(t)$
	\end{enumerate}
