...
\footnote{For the sake of generality, at this point we do not require the satisfiability propagator to be constructive. Below, we will then work with algorithms computing the values only required to exist, here.}
  ...
  \begin{proof}
    The pre-satisfiability certificate $I$ satisfies the quantifier free part. We will show that there is an extension of $I$ satisfying the quantified part. For this we use the principle of induction on well-orders to show that for every instantiation~$\psi$ of a quantified-subformula, the set of formulas $\{ \psi' \mid \psi'\prec \psi\}$ is satisfied by an extension of $I$. Due to compactness\todoSR{check details} this implies that the formula is satisfiable.
