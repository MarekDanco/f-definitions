...
% Mikol\'a\v s and I discovered the following trick.
...
To deal with disjunctions, Mikol\'a\v s proposed we could work with preconditions
(possible in \cite{kuncak-pldi10} or \cite{kuncak-sttt13}).
For each literal in disjunction we can form a precondition on $x$ in a way that
if the precondition holds, that literal has to be satisfied.\mj{Does it
satisfied the precondition are not?}
...
\begin{example}
	\[
		...
		\,\land \,\forall{x}. \,f(x) < f(x + 1)
	\]
	Since $f$ is increasing there has to exist an $a$ for which $f(a) \geq 0$\mj{do we need induction for this?}.
...
In the solution of the previous example\mj{always refer to the example by ref command, I don't know which you mean here.} we implicitly used the fact that the following
...
because in each $\alpha$ we can set $c$ to be outside $I$\mj{Algorithm can be applied, it just doesn't terminate.}. The same goes for any instantiation method.
