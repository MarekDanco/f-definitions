...
\begin{proof} \todoSR{I still need to check some details, e.g., the case $r<2$.}
	...
	\end{description}
	In all cases, the existential entity is neither assigned by $\alpha$ nor appears in any earlier instantiation. The 
eq condition ensures the existence of suitable propagators. Therefore, the construction forms a valid satisfiability certificate as defined in Definition \ref{def:certificate}.
\end{proof}
...
\mj{Potentially, in the implementation, use cores to determine which way to increase the size of~$I$.
	If a certain condition does not appear in the core, it might be possible to not
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $\text{condition} \leftarrow \bigwedge_{t \in \args(F,f)} \underline{I} + c^{f_{i^{*}}}_1 \leq t \leq \overline{I} + c^{f_{i^*}}_{l_{i^*}}$
		      ...
		      \begin{itemize}
			      \item Line 9. This will succeed because we can construct a mnodel as follows:
			            \begin{itemize}
				            ...
				            \item use the \req propagators to extend the instantiations in $[-N,N]\setminus[K_\text{min},K_\text{max}]$)
			            \end{itemize}
			      \item Line 5: $\smt(F \wedge \bigwedge_{x=-N}^N Q \wedge C)$ where $C$ is the clash condition $\forall t\in\args(F,f_{i^*}).-N+c_1^{f_{i^*}}\leq t\leq N+c_{l_{i^*}}^{f_{i^*}}$. This will succeed because the model constructed above also satisfies $C$ (followed from Claim \ref{claim:inTerminationProof} and the fact that $[K_{\text{min}},K_{\text{max}}]\subseteq[-N,N]$).
			      \item $\therefore$ Algorithm \ref{alg:sr} returns \textsc{Sat}.\qedhere
		      \end{itemize}
	\end{enumerate}
\end{proof}
