...
Let $f_1, \dots, f_r$ be the uninterpreted function symbols appearing in $Q$, with $T_i = \{t^{f_i}_1, \dots, t^{f_i}_{l_i}\}$ being the offset terms for $f_i$. We say that $Q$ satisfies the \emph{multi-\req} condition\todoSR{We are considering to make the definition parametric in $S_1,\dots,S_r$, $S_1',\dots,S_r'$ to allow for different variants of the algorithm.} if:
...
\begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item \emph{Propagability condition $\Phi_{\text{prop-up}}$:}\todoSR{The old ReqPivot is the special case where all $S_i$ are empty except for one, and this one contains exactly one element}
