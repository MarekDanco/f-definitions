...
\begin{equation}\label{eq:conj1}
    ...
    \,\land \,f(x + 3) > f(x)
\end{equation}
Formula~\eqref{eq:conj1} does not satisfy the ReqPivot condition because
...
\]
We can then merge the two universal quantifiers into a single formula
\begin{equation}\label{eq:normalized}
    \forall x. \,f(x) > f(x - 7) - f(x - 4)
    \,\land \, \,f(x) > f(x - 3)
\end{equation}
After normalization, the condition \emph{ReqPivot} holds for
...
cycle of Algorithm~\ref{alg:sr}, since it reduces the overall difference between the
smallest and biggest offset across multiple conjuncts.
...
with preconditions. For each disjunct we can form a precondition on $x$ in a way that
...
Here, both of the disjuncts satisfy \req. That means we can form complementary preconditions
