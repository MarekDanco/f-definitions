...
Given an entity interpretation $I$, we set $\mathit{def}(I)$ the set of entities for which $I$ is defined. Given a term $t$ and an entity interpretation $I$ such that  $\mathcal{I}(t)$ is unique for every $\mathcal{I}$ compatible with $I$, we define $I(t):=\mathcal{I}(t)$, and otherwise $I(t):=\bot$.\todoSR{This is not completely formal, we could also use a more formal, but less readable version.}
...
For example, $I=\{ c\mapsto 5 \}$ is a pre-satisfiability certificate of $c\geq 5 \wedge \forall x \;.\; f(x+c+1)= f(x+c)+1$, $\Gamma_{I,10}(\phi)=\{ f(15), f(16)\}$.
...
\begin{proof}
	...
	The corresponding pre-satisfiability certificate $I$ satisfies the quantifier free part of $\phi$. We use the principle of induction on well-orders to show that for every $z\in\mathbb{Z}$ the set of formulas $\{ \subst{Q}{x}{\repr{z}'} \mid z'\preceq z\}$ is satisfied by an extension of $I$.

	Let $z\in\mathbb{Z}$, and let $\Phi$ be $\{ \subst{Q}{x}{\repr{z}'} \mid z'\prec z\}$. We assume that $\Phi$ is satisfied by an extension $I_{\Phi}$ of $I$, and prove that $\Phi\cup \{ \subst{Q}{x}{\repr{z}}\}$ is satisfied by an extension of $I$, as well. Since the formula has a satisfiability certificate, the existential entities of $\psi$ do not occur in any element of $\Phi$. Let $I'$ be the extension of $I_{\Phi}$ that assigns to the existential entities $X_z$ the values whose existence is  confirmed by the satisfiability propagator. Since these do not occur in $\Phi$, for all entities occurring in $\Phi$, the value of $I'$ is the same as the value of $I_{\Phi}$. Hence $I'$ satisfies both $\Phi$ and $\subst{Q}{x}{\repr{z}}$, and so it satisfies $\Phi\cup \{ \subst{Q}{x}{\repr{z}}\}$.
...
Now, for any entity $u$, let $inst(u)$ either be the $z\in\mathbb{Z}$ with $u\in X_z$ or $\bot$ if there is no such instantiation (this means that either $I(u)\neq \bot$ or ...).
