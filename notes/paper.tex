...
\begin{tabbing}\hspace*{0.4cm}\=\hspace*{0.4cm}\=\kill
	...
	\textbf{return} $prop_{u,inst(u)}(I\cup \{ v\mapsto val(v) \mid v\in \Gamma_{I,inst(u)}(\phi)\setminus \mathit{def}(I)\setminus X_z\})$
\end{tabbing}

Here, the recursive calls go down the order $\prec$ in the following sense:
\begin{property}
	Let $u$ be an entity such that $inst(u)\neq\bot$. Then for any $v\in\Gamma_{I,inst(u)}(\phi)\setminus \mathit{def}(I)\setminus X_z$ with $inst(v)\neq\bot$, $inst(v)\prec inst(u)$.
\end{property}
