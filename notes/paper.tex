...
\begin{algorithm}
    \caption{Algorithm Wild Sketch (Likely Wrong)}
    \begin{algorithmic}[1]\label{alg_sketch}
        \Require $\phi = \forall x. \,\psi(x, f(x + c_1), \dots, f(x + c_n))$, ground part $\textit{G}$
        \Ensure A satisfying assignment to $f$
        \State $\textit{I} \gets \text{interpretation that grounds the arguments of } f$
        ...
        \State From $\phi_2$ we can get the propagator for the direction to the right
