...
for complex systems. Yet, counterexamples are important during modelling as
...
\begin{equation}
	f(a)\neq f(a+1)\land\forall x:\mathbb{Z}.\,f(x)<f(x+1)
\end{equation}

The formula has trivial counter-models, e.g., $f$ being the
identity function. Such solution can be found by synthesis approaches, e.g.,
\sygus.  But it is  easy to get out of the reach of these tools because they
are primarily based on templates end  candidate-solution enumeration. The
following modification is no longer solved by cvc5~\cite{cvc5} in its \sygus mode.
The function is required to increase in even steps and otherwise stay the same,
and the user wrongly assumes that there may be two values one apart that are
the same.\mj{Maybe even a simpler example?}
\begin{align*}
	      & f(a)\neq f(a+2)                                                                              \\
	\land & \forall x:\mathbb{Z}.\,2\mid x \Rightarrow f(x)<f(x+1)\land 2\nmid x \Rightarrow f(x)=f(x+1)
	% \land &   \forall x:\mathbb{Z}.\,2\mid x \Rightarrow f(x)<f(x+1)\\
	% \land &   \forall x:\mathbb{Z}.\,2\nmid x \Rightarrow f(x)=f(x+1)
\end{align*}

This paper shows that these formulas fall into a class were the desired model is
constructed by inductively defining functions on preceding values.

% \cite{ge-cav09}
% \cite{habermehl-etaps08}
% \cite{bradley-manna07,bradley-vmcai06}

\emph{Goal}: Deciding satisfiability of UFLIA (or UFNIA) formulas in a fragment which allows
the arguments of non constant uninterpreted function symbols to be only of the form
$x + k$, where $x$ is a universally quantified variable and $k$ is a numeral.
In the following, a formula will always refer to a UFLIA (or UFNIA) formula
whose only quantifiers are universal.
...
\begin{definition}[Simplified]
	...
	\begin{itemize}
		...
		\item for each instantiation $\psi$ of the quantified part of $\phi$ a set of corresponding
		      ...
		      or in any instantiation $\psi'$ with $\psi' \prec \psi$,
...
For this, let $inst(u)$ either be the  instantiation $\psi$ of a quantified sub-formula with $u\in\Gamma_I(\psi)$ and where $u$ is existential, or $\bot$ if there is no such instantiation\marek{How can this happen? We require all arguments to be in the form $x + k$, so for every entity there is a minimal $\psi$ that contains it. From minimality this $u$ has to be existential in $\psi$. Implicitly $u$ isn't in the ground part.}. For any instantiation $\psi$ of a quantified sub-formula with existential entities $u_1,\dots, u_s$ and non-existential entities $v_1,\dots, v_r$, the satisfiability propagator defines  functions $valprop_{u_1},\dots, valprop_{u_s}$ such that for any $\hat{v}_1,\dots,\hat{v}_r$, the entity interpretation
