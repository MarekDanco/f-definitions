...
Let $\mathcal{M}$ be a model of $\phi$ and extract the corresponding entity interpretation $I^*$.
...
Let $N = \max(|b_{\min}|, |b_{\max}|)$. Algorithm~\ref{alg:interval-check-param-withBigvee} expands the 
interval symmetrically from $[0,0]$. When it reaches iteration $n = N$, the interval becomes 
$[-N, N]$, which contains $B$.
...
Since $[-N, N]$ contains $B$, and $I^*$ satisfies all three conditions as shown above, this formula 
is satisfiable (witnessed by $I^*$). Therefore, the algorithm returns \textsc{Sat}.
\end{proof}
...
\item The formula used in \cref{alg:interval-check-param-withBigvee} is not in CNF, which is required by SMT solvers. We can either call the solver separately on the individual disjuncts of $\Psi_{\text{up}}([b_{\min},b_{\max}])$ and $\Psi_{\text{down}}([b_{\min},b_{\max}])$, or transform it into CNF using auxiliary variables.
...
\subsection{Handling Disjunction Using Auxiliary Propositional Variables}
...
\begin{itemize}
    ...
    \end{align*}
    where $\bar{u}_i = \{u_{i,j} \mid \neg s_{i,j}\}$ denotes a set of variables $u_{i,j}$, one for each offset $t_j^{f_i}$ whenever $s_{i,j}$ is false. The formula $Q'$ is obtained from $Q$ by simultaneously substituting:
    \begin{itemize}
...
\end{enumerate}
% Moreover, from a satisfying assignment to $\Psi_\text{up}^\text{e}(B) \land \Psi_\text{down}^\text{e}(B)$, we can extract witnessing subsets $\mathcal{S},\mathcal{S}'$ by setting $S_i = \{t_j^{f_i} \mid s_{i,j} \text{ is true}\}$ and $S'_i = \{t_j^{f_i} \mid s'_{i,j} \text{ is true}\}$.
\end{lemma}
\begin{proof}
