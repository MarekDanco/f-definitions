...
\newcommand{\mj}[1]{\todo[color=green!20]{#1 -mik}}
...
We will satisfy the universal quantifier by an inductive argument going both up and down. For making this argument work, we require that for one of the $f_1,\dots,f_r$, for both its highest argument $f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$ and lowest argument $f_{i^*}(x+c^{f_{i^*}}_1)$ we can always find values satisfying $Q$. More formally, there must be an $i^*\in \{1,\dots, r\}$ such that
\begin{itemize}
...
\mj{Is this condition really correct? If $F$ is empty, there's no condition on
	...
	and the positive)?}
\marek{When $F$ is empty, if the algorithm finds an $\alpha$, it concludes we can define all
	the remaining entities by propagation. We can currently do that only in the case when
	there is just one quantified assertion. This is addressed in \ref{todo:marek}.}
...
\section*{TODO notes}\label{todo:marek}

I agree that with the current wording the requirement \emph{ReqPivot} doesn't hold for
$Q = f(x)<f(x+1)\land f(x+1)<f(x+3)$, since not for all values of $f(x), f(x+1)$ can we find
a value of $f(x+3)$ that satisfies $Q$. Similarly for the second point.
However, in this case we can simply require that the values of the (non-existential)
entities in $Q$ are such that the literals not containing $f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$,
$f_{i^*}(x+c^{f_{i^*}}_1)$ in the first, respectively the second point, are satisfied.

Mikol\'a\v s found  another class of formulas where the current approach doesn't work.
Consider the following problem
\[
	\forall x. \,f(x + 7) > f(x) - f(x + 3)
	\,\land \,\forall x. \,f(x + 3) > f(x)
\]
Setting $f(x + 7) = f(x) - f(x + 3) + 1$ and $f(x + 3) = f(x) + 1$ leads to a contradiction,
because after 3 ``propagation steps'', starting from $x = 0$, in the first conjunct we get
$f(21) = 0$, while after 7 steps in the second conjunct we get $f(21) = 7$.

For that reason it would be useful to impose constraints on the definitions $F_{x + 7}, F_{x + 3}$
of $f(x + 7), f(x + 3)$, namely that $F^3_{x + 7} = F^7_{x + 3}$. These constraints are
likely necessary in the general case to maintain compatibility across the definitions,
since in the benchmarks the uninterpreted function symbols occur in
various quantified assertions.

Note that even with the current restrictions the problem
when working with multiple quantified formulas might be too hard.
Consider for example
\[
	\forall{x}. \,f(x) < f(x + 1) - f(x + 2)
	\,\land \,\forall{x}. \,f(x) < f(x + 1)
\]
The idea we use is that we first find \emph{I} where we can define $f$ and then proceed by propagation.
Let us instantiate by 0 -- we obtain the following system of inequalities
\begin{align*}
	f(0) & < f(1) - f(2) \\
	f(0) & < f(1)
\end{align*}
We can find a feasible region $R_1 \cup R_2$ by splitting on the sign of $f(2)$, where
\begin{align*}
	R_1 & = \{ (f(0), f(1), f(2)) \,| \,f(2) < 0 \,\land \,f(0) < f(1) \}           \\
	R_2 & = \{ (f(0), f(1), f(2)) \,| \,f(2) \geq 0 \,\land \,f(0) < f(1) - f(2) \}
\end{align*}
We would like to expand a model of this region into a model of the input formula.
Trivially, we cannot do that with $R_1$ since $f(2) < 0$ forces $f$ to be negative everywhere
and at the same time $f$ is increasing -- such function cannot be built over the integers.
We cannot expand a model of $R_2$ either, since if we instantiate the condition
\[
	\forall{x}. \,f(x) < f(x + 1) - f(x + 2)
\]
by 0 and 1, we get $f(0) < -f(3)$, which together with $f(0) < f(3)$ and the fact that
$f$ is forced to be non-negative in $R_2$ yields a contradiction. Therefore, the problem is
unsatisfiable.

The problem in showing that $R_1$ cannot be used in finding the model is that it requires
proving the following set of formulas is $\textsc{unsat}$ over the integers
\[
	\forall{x}. \,f(x)<0 \,\land \,\forall{x}. \,f(x)<f(x+1)
\]
This is already too hard for SMT solvers, because it cannot be proven by instantiation
\footnote{In general, we can look for bounds of the form $\forall x. \,f(x) < G$ for $G$ ground.
	If from the other quantified subformulas we can derive that $f$ is stricly increasing, we can conclude
...
Mikol\'a\v s pointed out that the above problem can be proven $\textsc{unsat}$ only by the schema of induction.
Using $\forall{x}. \,f(x)<f(x+1)$ we can prove that
\[
...
\]
where the second implication holds by the assumption that $f$ is increasing. Instantiating the lemma by $v = -f(0)$
now yields {unsat}, since we get
\[
	\forall{x}. \,f(x)<0 \land \exists{x}. \,f(x) \geq 0
\]

Another example when induction is needed is
\[
	f(0) = 0 \,\land \,f(c) = 3 \,\land \,\forall{x}. \,f(x+1) = f(x)
\]
Note that (the extension to unsatisfiable formulas of) our algorithm cannot be applied in this case
because in each $\alpha$ we can set $c$ to be outside $I$. The same goes for any instantiation method.
