...
\begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item \emph{Propagability condition $\Phi_{\text{prop-up}}$:}\todoSR{Stefan N. will add an example, the old ReqPivot is the special case where all $S_i$ are empty except for one, and this one contains exactly one element}
        \[\forall x \forall \bar{u}_1 \dots \forall \bar{u}_r \exists v_1\dots \exists v_r.\ Q'\] where $Q'$ is the result of replacing in $Q$ for all $i\in \{1,\dots, r\}$,
        \begin{itemize}
        \item for all $t\in S_i$, $f_i(x+t)$ with $v_i$, and
        \item for all $t\in T_i\setminus S_i$, $f_i(x+t)$ with $u_i^t$, and where
        \end{itemize}
    ...
    \begin{enumerate}
      ...
      \item  \emph{Propagability condition $\Phi_{\text{prop-down}}$:} same as $\Phi_{\text{prop-up}}$, but for $S_1',\dots, S_r'$
