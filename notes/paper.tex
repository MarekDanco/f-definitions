...
For each $l_i$ we may attempt to form a precondition $\mathit{pre}_i$ so that
$\forall x. \, \mathit{pre}_i(x) \implies l_i(x)$ is valid.
...
now proves the Formula~\eqref{eq:ind-needed} is \textsc{unsat}, since we get
\[
...
\[
	f(0) = 0 \,\land \,f(c) = 3 \,\land \,\forall{x}. \,f(x+1) = f(x)
\]
Note that applying Algorithm~\ref{alg:sr} results in an infinite loop
