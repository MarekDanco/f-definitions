...
\begin{itemize}
	...
	\item there is no $t\in args(F, f)$, $x<\min I$  s.t.\ $x+c^{f_{i^{*}}}_1=\alpha(t)$ (no entity propagated to by downward induction clashes with an argument of $f$ in $F$)
        \end{itemize}
        Then the order $\max I+1, \max I+2, \dots, \min I-1, \min I-2$, together with the entity interpretation $\alpha$, and propagators for ReqPivot form a satisfiability certificate for $F\wedge \bigwedge_{x\in I} Q\wedge \forall_{x\not\in I} Q$.
\end{property}
...
be formed as value expressions $A \leq B \,\land \,B \leq A$. This cannot be solved by \cite{bradley-manna07, ge-cav09} though.
...
Mikol\'a\v s and I discovered the following trick. Suppose we have this formula
\[
...
cycle of Algorithm \ref{alg:sr}. Note that this doesn't work for disjunction.
...
Here we pick exactly one disjunction based on the parity of $x$. We never assign to the same entity
twice, because the difference between the offsets is even.

\[
\forall{x}. \,f(x + 3) > f(x + 7) \,\lor \,f(x + 4) < f(x + 5)
\]
Precondition for each disjunct is $\top$.
...
In the solution of the previous example we implicitly used the fact that the following
formula is \textsc{unsat}
\[
	\forall{x}. \,f(x)<0 \,\land \,\forall{x}. \,f(x)<f(x+1)
\]
Note that one cannot prove that by instantiation\footnote{
...
Indeed, every finite set of instantiations of the above formula is satisfiable, therefore the problem
cannot be proven by instantiation.
...
Using $\forall{x}. \,f(x)<f(x+1)$ we can prove by induction that
\[
...
now proves the formula is \textsc{unsat}, since we get
\[
