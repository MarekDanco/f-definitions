...
Let $f_1, \dots, f_r$ be the uninterpreted function symbols appearing in $Q$, with $T_i = \{t^{f_i}_1, \dots, t^{f_i}_{l_i}\}$ being the offset terms for $f_i$. We say that $Q$ satisfies the \emph{multi-\req} condition\todoSR{I (SR) am considering to make the definition parametric in $S_1,\dots,S_r$, $S_1',\dots,S_r'$ to allow for different variants of the algorithm.} if:
...
\begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item \emph{Propagability condition $\Phi_{\text{prop-up}}$:}\todoSR{Stefan N. will add an example, the old ReqPivot is the special case where all $S_i$ are empty except for one, and this one contains exactly one element}
        \[\forall x \forall \bar{u}_1 \dots \forall \bar{u}_r \exists v_1\dots \exists v_r.\ Q'\] where  $Q'$ is the formula obtained from $Q$ by simultaneously replacing for all $i\in\{1,\dots,r\}$,
        \begin{itemize}
            ...
            % $\bar{u}_i$ stands for all $u_i^t$ with $t\in T_i\setminus S_i$.            
        \end{itemize}

    \end{enumerate}
    
    \item \textbf{Downward propagation:} There exist subsets $S'_1 \subseteq T_1, \dots, S'_r \subseteq T_r$ (some possibly empty) such that the following two conditions hold:
    \begin{enumerate}
      ...
      \item  \emph{Propagability condition $\Phi_{\text{prop-down}}$:}\\
      Defined analogously to $\Phi_{\text{prop-up}}$, with $S'_i$ in place of $S_i$.

        % Let $v_1, \dots, v_n$ enumerate all function applications in $Q$ that are NOT of the form $f_i(x+t)$ with $t \in S'_i$. Then $\forall x \forall c_1 \dots \forall c_n \exists w.\ Q'$ where 
    %     \begin{equation*}
...
\[
\forall x.\ Q(x) \quad \text{where} \quad Q(x): f(x+c+3) + f(x+4) + f(x+1) = g(x+2) + h(x)
\]
...
\begin{enumerate}
    ...
    \[
    S_f = \{4\}, \quad S_g = \{2\}, \quad S_h = \emptyset.
    \]
    \begin{enumerate}
        \item \textbf{Maximality $\Phi_{\text{max}}$:}
        \begin{itemize}
            \item For $f$: $S_f = \{4\}$, $T_f \setminus S_f = \{c+3, 1\}$, so we need:
            \[
            4 = 4 \quad\text{and}\quad 4 > c+3 \ \land\ 4 > 1
            \]
            $4 > 1$ is true. $4 > c+3$ becomes a constraint: $c < 1$.
            \item For $g$: $S_g = \{2\}$, $T_g \setminus S_g = \emptyset$, so $2=2$ (true).
            \item For $h$: $S_h = \emptyset$, vacuously true.
        \end{itemize}
        Thus $\Phi_{\text{max}}$ holds provided $c < 1$.

        \item \textbf{Propagability $\Phi_{\text{prop-up}}$:}
        We introduce:
        \begin{itemize}
            \item $v_f$ for $f(x+4)$
            \item $v_g$ for $g(x+2)$
...
\paragraph{Choice for downward: Take the numerically minimal offset}
\[
S'_f = \{1\}, \quad S'_g = \emptyset, \quad S'_h = \{0\}.
\]

\begin{enumerate}[label=(\alph*)]
    \item \textbf{Minimality $\Phi_{\text{min}}$:}
    \begin{itemize}
        \item For $f$: $S'_f = \{1\}$, $T_f \setminus S'_f = \{c+3, 4\}$, so we need:
              \[
              1 = 1 \quad\text{and}\quad 1 < c+3 \ \land\ 1 < 4
              \]
              $1 < 4$ is true. $1 < c+3$ gives $c > -2$.
        \item For $h$: $S'_h = \{0\}$, $T_h \setminus S'_h = \emptyset$, so $0=0$ (true).
        \item For $g$: $S'_g = \emptyset$, vacuously true.
    \end{itemize}
    Thus $\Phi_{\text{min}}$ holds provided $c > -2$.

    \item \textbf{Propagability $\Phi_{\text{prop-down}}$:}
    We introduce:
    \begin{itemize}
        \item $v'_f$ for $f(x+1)$
        \item $v'_h$ for $h(x)$
...
$Q$ satisfies multi-\req if we can find $c$ satisfying both:
\begin{itemize}
    \item For upward (Choice 1): $c < 1$
    \item For downward: $c > -2$
\end{itemize}
Thus any $c \in (-2, 1)$ works. If $c \geq 1$, we could use Choice 2 for upward ($c > 1$).
\end{example}
