...
\begin{definition}[Offset Term]
    An \emph{offset term} is a term of the form $x + k$, where $x$ is a variable, $k\in\mathbb{Z}$.
\end{definition}

\begin{definition}[Write to a Cell]
    We say a formula $\varphi[x\mapsto a]$ \emph{writes to a cell} $f(n)$ for $n\in\mathbb{Z}$, when $\varphi$ contains an offset term $t(x)$
    such that $t(x\mapsto a) = n$ and $f(t)$ appears in $\varphi$.
\end{definition}
...
$\forall x. \,l_1(x) \lor \dots \lor l_n(x)$, where the arguments of $f$ in
each disjunct $l_i(x)$ are offset terms.
If we can construct formulas $\mathit{pre}_1, \dots, \mathit{pre}_n$ so that
\begin{itemize}
    \item $\forall x. \,\bigvee_{i = 1}^n \mathit{pre}_i(x)$ is valid,
    \item there are no $i, j$ such that $\exists x_1, x_2. \,\mathit{pre}_i(x_1) \,\land \,\mathit{pre}_j(x_2)$
          and $l_i[x \mapsto x_1]$, $l_j[x \mapsto x_2]$ write to the same cell,
\end{itemize}
...
is satisfied. Notice we never assign to the same entity twice, because the difference between the offsets is even.
...
\marek{If we can form complementary preconditions, then we can do the normalization trick. Add the case
    when the above formula is unsat. No hope for \req . Finding preconditions is hard.}
...
\begin{example}
    ...
    \[
        f(x) = f(x + 1) + f(x + 2) -1
    \]
\end{example}\marek{\req satisfied}
...
\end{example}\marek{\req not satisfied. Better motivation for induction.}
