...
We say\todoSN{I am not sure whether we need this or just directly saying the sentence after the two bullet points?} that $Q$ satisfies the \emph{multi-\req condition for $(\mathcal{S}, \mathcal{S}')$} if:
\begin{itemize}
...
We say $Q$ satisfies the \emph{multi-\req condition} (without parameters) if there exists \emph{some} pair $(\mathcal{S}, \mathcal{S}')$ for which the above holds.
\end{definition}
...
\cref{thm:interval-certificate-param} immediately yields an algorithm that searches for an interval $B$ satisfying the three conditions. To use the parameterised version, the algorithm can iterate over candidate pairs $(\mathcal{S},\mathcal{S}')$.
...
We keep \cref{alg:interval-check-param} as simple as possible, not taking into account efficiency considerations. We will now turn to efficiency.
