    ...
    \begin{itemize}
        ...
        \item for each instantiation of a quantified sub-formula of $\phi$
              \begin{itemize}
                  ...
                  \item a certificate showing that for all values of the non-existential entities there exist values for all other entities satisfying this instantiation, which we will call \emph{satisfiability propagator}\footnote{The satisfiability propagators typical have the form entity=term, where the left-hand sides are the existential
variables. But at some point, we might allow satisfiability propagators of a different form, for example,
...
The goal is not a constructive converse for certain classes of formulas.\footnote{A converse shows that a satisfiability certificate exists for such a  class, and a  constructive converse, in addition, provides an algorithm that finds this certificate}
