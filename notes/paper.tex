...
\begin{property}
	...
	\end{itemize}
	Then the order $0,1,-1,2,-2\dots, I, -I,\max I+1, \max I+2, \dots, \min I-1, \min I-2,\dots$, together with the entity interpretation $\alpha$, and the propagators resulting from $\alpha$ (for $x\in I$), and from ReqPivot (for $x\not\in I$) form a satisfiability certificate for $F\wedge \forall x \:.\: Q$.
\end{property}

\begin{proof} (under development)
	Since $\alpha\models F\wedge \bigwedge_{x\in I} Q$, for all $x\in I$, $\alpha$ assigns values to all elements of $\Gamma_{\alpha}(Q(x))$, and hence $\alpha$ itself plays the role of the satisfiability propagator.  For $x>\max I$,  neither $\alpha$, nor the propagators for $x'= \max I+1,\dots ,x-1$ assign a value to  $f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$, and hence the propagator from the first condition from $ReqPivot$ is correct. For $x<\min I$,  neither $\alpha$, nor the propagators for $x'= \max I+1,\dots$, nor the propagators for $x'=\min I-1,\min I-2,\dots, x+1$ assign a value to  $f_{i^*}(x+c^{f_{i^{*}}}_1)$, and hence the propagator from the second condition from $ReqPivot$ is correct.
\end{proof}
% corresponding existential entities:
%  \begin{enumerate}
%  \item none (all entities are assigned a values by $\alpha$)
%  \item $f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$, $x= \max I+1, \max I+2,\dots$
%  \item $f_{i^*}(x+c^{f_{i^*}}_1)$, $x=\min I -1, \min I-2, \dots$
%  \end{enumerate}
