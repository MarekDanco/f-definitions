...
Let $B = [b_{\min}, b_{\max}]$ be a non‑empty integer interval.  
Then $\phi = F \land \forall x.\, Q$ has a satisfiability certificate — and hence is satisfiable — provided that
\[
\Bigl(F \land \bigwedge_{x \in B} Q[x]\Bigr) \;\land\; \Psi_{\text{up}}(B) \;\land\; \Psi_{\text{down}}(B)
\]
holds.
\end{theorem}
...
\[
F \land \bigwedge_{x \in B} Q[x]\;\land\; \Psi_{\text{up}}(B) \;\land\; \Psi_{\text{down}}(B).
\]
From $I \models \Psi_{\text{down}}(B)$, pick subsets $S_1^{\downarrow}, \dots, S_r^{\downarrow}$ that witness its disjunct. We construct a certificate as in \cref{def:certificate}:
