...
Given a term $t$ and an entity interpretation $I$ such that  $\mathcal{I}(t)$ is unique for every $\mathcal{I}$ compatible with $I$, we define $I(t):=\mathcal{I}(t)$, and otherwise $I(t):=\bot$.\todoSR{This is not completely formal, we could also use a more formal, but less readable version.}
...
For this, for any entity $u$, let $inst(u)$ either be the  instantiation $\psi$ of a quantified sub-formula with $u\in\Gamma_I(\psi)$ and where $u$ is existential, or $\bot$ if there is no such instantiation\marek{How can this happen?}\todoSR{It can happen with function symbols of arity $\geq 2$.}
