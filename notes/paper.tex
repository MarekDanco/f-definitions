...
\begin{definition}[Satisfiability Certificate]
    ...
    \begin{itemize}
        \item an order\footnote{For now, I assume this order to be total.} on all instantiations
              \todo{Do you mean an order on the constraints these instantiations produce? -Marek} of quantified
...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \State $\psi_2 \gets \bigvee_{c \in S} \forall x. \,\varphi(x)[c]$
        \Comment{eliminite $y$ from $\psi_1$}
        \State $\psi_3 \gets \bigwedge_{d \in T}\bigvee_{c \in S}  \,\varphi[d, c]$
        \Comment{eliminite $x$ from $\psi_2$}
        \State $\{(d_1, c_1), \dots, (d_n, c_n)\} \gets$ values of $c, d$ in the true literals of $\psi_3$
