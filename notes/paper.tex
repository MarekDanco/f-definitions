...
We will satisfy the universal quantifier by an inductive argument going both up and down. For making this argument work, we require that for one of the $f_1,\dots,f_r$, let us say for $f_{i^*}$, for both its highest argument $f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$ and lowest argument $f_{i^*}(x+c^{f_{i^*}}_1)$ we can always find values satisfying $Q$. To formalize this, for every sub-term of $Q$ of the form $f_i(t)$, where $t$ is an arbitrary term, we introduce a fresh integer variable $\nu_i^t$ and denote by $\mathcal{V}$ the set of all these variables. We denote by $Q^\nu$ the result of replacing all these sub-terms of $Q$ by the corresponding variable from $\mathcal{V}$.
Now there must be an $i^*\in \{1,\dots, r\}$ such that\todoSR{I tried to write \req in a way that is formally sound. I am not really satisfied with the result, so any alternative idea is very welcome.}
...
\begin{itemize}
\item $\forall \nu_1\dots \forall_{\nu_{|\mathcal{V}|-1}}\exists \nu_{i^*}^{x+c^{f_{i^*}}_{l_{i^*}}} .\, Q^\nu$, where $\nu_1,\dots,\nu_{|\mathcal{V}|-1}$ are all variables in $\mathcal{V}$ except for $\nu_{i^*}^{x+c^{f_{i^*}}_{l_{i^*}}}$.
...
% More formally, there must be an $i^*\in \{1,\dots, r\}$ such that
% % \mj{Just to be clear, the requirement above does not apply to $f(x)<f(x+1)\land f(x+1)<f(x+3)$?}
% % \todoSR{Correct. It applies to both parts individually, but not to the conjunction. This is  restrictive, of course, but allowed for an easy generalization to the case of several occurrences of the same function symbol with different arguments.}
% \begin{itemize}
