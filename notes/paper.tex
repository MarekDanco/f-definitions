...
We will satisfy the universal quantifier by an inductive argument going both up and down. For making this argument work, we require that for one of the $f_1,\dots,f_r$, for both its highest argument $f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$ and lowest argument $f_{i^*}(x+c^{f_{i^*}}_{l_{1}})$ we can always find values satisfying $Q$. More formally, there must be an $i^*\in \{1,\dots, r\}$ such that \mj{the lowest should be $c^{f_{i^*}}_1$?}
\begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%
I added some notes in \ref{todo:marek}. -Marek
%%%%%%%%%%%%%%%%%%%%%%%%
...
It is good to note that even with the current restrictions the problem
...
\begin{align*}
	...
	f(0) & < f(1)
\end{align*}
We can find a feasible region $R_1 \cup R_2$ by splitting on the parity of $f(2)$, where
\begin{align*}
...
and at the same time $f$ is increasing -- such function cannot be built over integers.
...
$f$ is forced to be positive in $R_2$, yields a contradiction. Therefore, the problem is
unsatisfiable.

The problem is that showing $R_1$ cannot be used in finding the model is that it requires
...
Namely, every finite set of instantiations is satisfiable.
