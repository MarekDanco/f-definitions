...
\begin{algorithm}
	...
	\label{alg:sr}
	\begin{algorithmic}[1]
		\Require Formula $F \wedge \forall x \:.\: Q$ as above
		\Ensure \textsc{sat} or \textsc{unsat}
		% \Procedure{OriginalSMT}{$F, f, k, l, Q$}
		\State $\ubar{I} \leftarrow 0$, $\bar{I} \leftarrow 0$ \Comment{Start with singleton interval}
		\State $\alpha \leftarrow \smt\left(F \wedge \bigwedge_{x=\ubar{I}}^{\bar{I}} Q\right)$

		\While{$\alpha \neq \bot$}

		\State $\text{condition} \leftarrow \bigwedge_{t \in \args(F,f)} \ubar{I} + c^{f_{i^{*}}}_1 \leq t \leq \bar{I} + c^{f_{i^*}}_{l_{i^*}}$
		...
		\If{for some $t$, $t \leq \bar{I} + c^{f_{i^*}}_{l_{i^*}} \in \mathcal{C}$}
		\State $\bar{I} \leftarrow \bar{I} + 1$
		\EndIf
		\If{for some $t$, $\ubar{I} + c^{f_{i^{*}}}_1 \leq t \in \mathcal{C}$}
		\State $\ubar{I} \leftarrow \ubar{I} - 1$
		\EndIf
	...
	\end{algorithmic}
\end{algorithm}
\marek{I think $c^{f_{i^{*}}}_1, c^{f_{i^*}}_{l_{i^*}}$ can be variables but we need to maintain their differences.}
...
\begin{equation}\label{eq:normalized}
	...
	\,\land \, \,f(x) > f(x - 3)
\end{equation}
Note that the condition \req now holds for
Formula~\eqref{eq:normalized} for upward propagation too, so Algorithm~\ref{alg:sr} can be applied.
...
\marek{TODO fix Algorithm~\ref{alg:sr}}
\marek{TODO write algorithm for constructing the model if satisfiable}
