...
\begin{definition}\todoSR{Sorry, I changed this once more. The intuition stays the same.}
	...
	\begin{itemize}
		...
		\item for every instantiation $\psi$ of a quantified sub-formula of $\phi$, a set of entities (the \emph{existential entities})\todoSR{alternative term ``propagated entities''} that are neither assigned a values by $I$, nor occur in any $\Gamma_I(\psi')$ for an instantiation $\psi'\prec\psi$
...
\begin{property}
	\label{prop:cert}
	Moreover, assume a non-empty integer interval $I$ and an entity interpretation $\alpha$ s.t.
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item         order: $0,1,-1,2,-2\dots, I, -I,\max I+1, \max I+2, \dots, \min I-1, \min
			      I-2,\dots$,
...
\begin{proof} \todoSR{I still need to check some details, e.g., the case $r<2$.}
	We prove that the existential entities are neither assigned a value by $\alpha$, nor occur in any $\Gamma_I(\psi')$ for an instantiation $\psi'\prec\psi$.
	For $x\in I$, the set of existential is empty, hence there is nothing to prove. For $x>\max I$, the existential entity~$f_{i^*}(x+c^{f_{i^*}}_{l_{i^*}})$ is neither allowed to be assigned a value by $\alpha$, nor allowed to occur in any instantiation $x'$ with $\max I<x'<x$. The former is ensured by the second item of Property~\ref{prop:cert}, the latter by the fact that $x+c^{f_{i^*}}_{l_{i^*}}$ is the highest argument of $f_{i^*}$.
	For $x<\min I$, the existential entity~$f_{i^*}(x+c^{f_{i^{*}}}_1)$, is neither allowed to be assigned a value by $\alpha$, nor allowed to occur in any instantiation~$x'$ with $\max I<x'$, nor by any $x'$ with $x<x'<\min I$. The first condition is ensured by the third item of Property~\ref{prop:cert}, the second condition by\todoSR{This is the ``glueing together'' mentioned by Mikoláš below. We need to require a minimal size for $I$ to avoid a problem, here.}\todoSN{Perhaps $\abs{I}\geq\Delta$, $\Delta=c_{l_{i^*}}^{f_{i^*}}-c_1^{f_{i^*}}$? Then we have the lowest argument is $(\max I+1) + c^{f_{i^*}}_1 \geq (\min I+\Delta) + c^{f_{i^*}}_1 + 1=\min I+c^{f_{i^*}}_{l_{i^*}}+1>(\min I-1)+c^{f_{i^*}}_{l_{i^*}}\geq x+c^{f_{i^{*}}}_1$}, and the third condition by the fact that $x'+c^{f_{i^{*}}}_1$ is the lowest argument of $f_{i^*}$.
\end{proof}
...
maximal (minimal) offset.
As a result, the algorithm effectively tries to satisfy the formula using
only this single disjunct, which isn't sound. Therefore, a different approach is required for handling disjunctions.
...
\marek{It is is very difficult for me to argue why these conditions are sufficient. I don't think they are.
...
\end{enumerate}
then we can solve instead the following formula
...
we try to write to $f(6)$ two times with different values.
