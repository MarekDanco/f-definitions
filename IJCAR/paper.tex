...
We use SMT terminology (which differs from classical logic by being typed, and defining notions such as satisfiability in a slightly different way). \todoSR{Mikoláš, are you aware of a standard way of describing those difference?}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item for every $z\in\mathbb{Z}^{\numvar{\bar{x}}}$
                  \begin{itemize}
                  \item a set $X_z\subseteq \Gamma_{I,z}(\phi)$  (the \emph{propagated cells}) such that \[X_z\cap \mathit{def}(I)=\emptyset, X_z\cap\bigcup_{z'\prec z} \Gamma_{I,z'}(\phi)=\emptyset,\] and
