...
We will now proceed from the  general certification framework to concrete computation. Throughout this section we assume a formula $\phi$ of the form~$F \land \forall \bar{x}.\; Q$, but with the additional restrictions described in the problem statement (\cref{sec:problem-statement}). In particular, we will allow only one quantified variables~$x$. The restrictions also require all arguments of any uninterpreted function symbol $f$ in $Q$ to have the same coefficient, and we denote the sign of this coefficient by $\coeffsign{f}$.
