...
\begin{itemize}
    ...
    % \item Upward propagation encoding: For the given Boolean variables $\{s_{f(t)}\}$, define $\Phi_{\text{prop-up}}^\text{e}(\{s_{f(t)}\})$ as:
    % \[
    %     \forall x \forall \bar{u} \exists \bar{v}.\ 
    %     \bigwedge_{f(t) \in \mathcal{T}} \Big((s_{f(t)} \Rightarrow (f(t) = v_{f(t)})) \land (\neg s_{f(t)} \Rightarrow (f(t) = u_{f(t)}))\Big) \Rightarrow Q
    % \]
    % where $\bar{u} = \{u_{f(t)} \mid f(t) \in \mathcal{T}\}$ and $\bar{v} = \{v_{f(t)} \mid f(t) \in \mathcal{T}\}$.
