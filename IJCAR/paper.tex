...
Intuitively, a satisfiability certificate encodes an induction proof, where the well-order~$\preceq$ sets the direction of induction and each step of the induction shows satisfiability of the universal quantifier for another choice $z$ for $\bar{x}$. Such a step is allowed to fix the values of the propagated cells~$X_z$. These cells must be chosen in such a way that these propagated cells neither have already been given a value by the pre-satisfiability certificate~$I$ nor by an earlier induction step.
