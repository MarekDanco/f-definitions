...
A pre-satisfiability  certificate interprets all arguments of uninterpreted function symbols in $F$ to a value in the domain $\domain$, and---together with a value for the quantified variables---also in $Q$. This fixes a set of cells.
\begin{definition}[Fixed Cells]
  For any $z\in\domain^{\numvar{\bar{x}}}$, $\Gamma_{I,z}(F\wedge \forall \bar{x}\;.\; Q):=$
