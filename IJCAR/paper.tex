...
In this paper, we address this problem for formulas containing universal quantifiers and uninterpreted function symbols over the integers, which corresponds to the SMTLIB logic UFLIA. This logic is undecidable~\cite{Downey:72,Horbach:17}. However, such formulas occur in important applications, for example, in program verification, where uninterpreted function symbols play an important role in reasoning about arrays. Current SMT solvers use heuristics\todoSR{reference?} to prove unsatisfiability of such formulas. However, they struggle to prove satisfiability.\todoSR{we need make this more precise}




\paragraph{What We Solve.} We therefore isolate a relevant fragment of UFLIA where formulas like~\eqref{eq:periodic} become decidable\todoSR{I am hesitating to use the term ``decidable'' since we require the \req-check}. In this fragment, arguments of uninterpreted functions within quantified subformulas are restricted to terms of the form $x + t$, where $x$ is the universally quantified variable and $t$ is a ground term. Such formulas frequently arise in verification conditions for programs with arrays, recursive functions, or inductive data structures.
