...
\begin{example}[A Satisfiability Certificate]\label{ex:certificate-example}
...
\begin{itemize}
    ...
    \item propagated cells $X_{(3,i)} = 
        \begin{cases}
            \{g(0)\} & i=0,\\
            \{f(3,i), g(i+1)\} & i>0,\\
            \{f(3,i), g(i)\} & i<0.
        \end{cases}$
    \item propagators: $g(0)\mapsto5$; for $i>0$: $f(3,i)=i+g(i)-1$, $g(i+1)=g(i)-2$.
\end{itemize}
\end{example}
...
We will now proceed from the  general certification framework to concrete computation. Throughout this section we assume a formula $\phi$ of the form~$F \land \forall \bar{x}.\; Q$, but with the additional restrictions described in the problem statement (\cref{sec:problem-statement}). We will introduce an algorithm that checks satisfiability of $\phi$  by computing a satisfiability certificate of a certain form. 
...
\item \textbf{Maximality condition $\Phi_{\text{max}}(\mathcal{S})$:}
  ...
  \[
        ...
        \right)}{x}{0}.
    \]
Symmetrically, for downward propagation, define the minimality condition $\Phi_{\text{min}}$ with $<$ instead of $>$.
    ...
    \item \textbf{Propagability condition $\Phi_{\text{prop}}(\mathcal{S})$:}
        ...
        \end{itemize}
\end{itemize}
We say that $Q$ satisfies the \emph{\req condition} for subsets $\mathcal{S}\subseteq\mathcal{T}$ and $\mathcal{S}'\subseteq\mathcal{T}$ iff $\Phi_{\text{max}}(\mathcal{S})$, $\Phi_{\text{prop}}(\mathcal{S})$,
$\Phi_{\text{min}}(\mathcal{S}')$, and $\Phi_{\text{prop}}(\mathcal{S}')$ hold. Moreover we say that $Q$ satisfies the \emph{\req condition} iff there exists a pair~$(\mathcal{S}, \mathcal{S}')$ such that $Q$ satisfies the \req condition for $(\mathcal{S}, \mathcal{S}')$.
\end{definition}
...
\begin{enumerate}
    ...
    \item \textbf{Propagability $\Phi_{\text{prop}}(\mathcal{S})$:}
        In $Q$, replace:
        \begin{itemize}
...
\begin{enumerate}
    ...
    \vspace*{-0.2cm}
    \[\land 
    \bigwedge_{f(t') \in \mathcal{T}\setminus\mathcal{S}} 
    \subst{t'}{x}{b_{\max}+1} \geq b_{\min}\]
    where $\Phi_{\text{max}}(\mathcal{S})$ and $\Phi_{\text{prop}}(\mathcal{S})$ are as defined in \cref{def:reqpivot-param}. 
    ...
    \]
    where $\Phi_{\text{min}}(\mathcal{S}')$ and $\Phi_{\text{prop}}(\mathcal{S}')$ are as defined in \cref{def:reqpivot-param}.
\end{enumerate}
\end{definition}

Thus, for any choice of subsets $\mathcal{S},\mathcal{S}'\subseteq\mathcal{T}$, $\Psi_{\text{up}}(B,\mathcal{S})$ (resp. $\Psi_{\text{down}}(B,\mathcal{S}')$) asserts that $\mathcal{S}$ satisfies the upward (resp.\ $\mathcal{S}'$ satisfies the downward) conditions of \req \emph{and additionally} satisfies the clash condition for interval $B$. The clash condition $a \leq \subst{t}{x}{\repr{b}_{\max}}$ ensures that for every ground argument $a$ of $f$ appearing in $F$, the propagated cell $f(\subst{t}{x}{\repr{z}})$ with $z > b_{\max}$ and $f(t)\in\mathcal{S}$ is different from the ground cell $f(a)$; similarly for downward propagation. 

The additional constraint $\bigwedge_{f(t') \in \mathcal{T}\setminus\mathcal{S}} (\subst{t'}{x}{\repr{b_{\max}+1}} \geq b_{\min})$ ensures that during upward propagation, every non‑propagated cell's argument lies at or to the right of $b_{\min}$. Since in the well‑order used in Theorem~\ref{thm:interval-certificate-param} all indices 
$z > b_{\max}$ are processed \emph{before} any $z < b_{\min}$, this guarantees that upward propagation never depends on cells that would be assigned later by downward propagation. Consequently, the inductive construction of the certificate remains well‑founded. No symmetric constraint is needed for $\Psi_{\text{down}}$ because downward propagation occurs later in the order and may safely use cells already assigned during upward propagation.
...
\[
  \Biggl(F \land \bigwedge_{x \in B} \subst{Q}{z}{\repr{x}}\Biggr) \;\land\;
...
\begin{enumerate}
    \item \textbf{Pre‑satisfiability assignment:} Define the cell interpretation~$I$ as follows:
      \begin{itemize}
        \item For every uninterpreted constant $c$ in $\phi$, set $I(c)=I^*(c)$.
        \item For every cell $f(n)$ where $f$ is an uninterpreted function symbol, if there exists a ground term $a$ in $F$ such that $I^*(a)=n$, then set $I(f(n))=I^*(f(a))$.
    \end{itemize}
    By construction, $I$ is finite and satisfies $F\land\bigwedge_{z\in B}Q[z] $, because $I^*$ satisfies that formula and $I$ agrees with $I^*$ on all relevant cells.
  ...
  \item \textbf{Propagated cells $X_z$ for $z \in \mathbb{Z}$:}
    ...
    X_z = 
    \begin{cases}
    \Gamma_{I,z}(\phi)\setminus \mathit{def}(I) & \text{if } z \in B, \\[4pt]
    ...
    \begin{itemize}
    \item For $z \in B$: In this case, $\Gamma_{I,z}(\phi)\setminus \mathit{def}(I)\setminus X_z$ is empty. The satisfiability propagator simply sets the values of $X_z$, which is $\Gamma_{I,z}(\phi)\setminus \mathit{def}(I)$ to the values given by $I^*$.
        \item For $z > b_{\max}$: the condition $\Phi_{\text{prop}}(\mathcal{S})$ provides, for any given values for all non‑propagated cells in $\Gamma_{I,z}(\phi)$ and each $f(t)\in\mathcal{S}$ values $v_{f(t)}$  such that assigning these values to each corresponding cells in $X_z$ satisfies~$\subst{Q}{x}{\repr{z}}$.
        \item For $z < b_{\min}$: Symmetrically, use $\Phi_{\text{prop}}(\mathcal{S}')$ to assign values to propagated cells.
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
    \item To prove that this element of $X_z$ cannot be in $\mathit{def}(I)$, let $f'(t')$ be an arbitrary, but fixed element of $\mathit{def}(I)$. We prove that $f'(I(\subst{t'}{x}{\repr{z}}))\neq f(I(\subst{t}{x}{\repr{z}}))$. If $f$ is a different function symbol than $f'$ this is certainly the case, and hence it suffices to prove
      $f(I(\subst{t'}{x}{\repr{z}}))\neq f(I(\subst{t}{x}{\repr{z}})$, which makes it necessary to prove
      $I(\subst{t'}{x}{\repr{z}}))\neq I(\subst{t}{x}{\repr{z}})$.
The clash condition $a \leq \subst{t}{x}{\repr{b}_{\max}}$ in $\Psi_{\text{up}}(B,\mathcal{S})$ guarantees that for every ground argument $a \in \args(F,f)$:
        \[
        I\models a \leq \subst{t}{x}{\repr{b}_{\max}} < \subst{t}{x}{\repr{z}},
        \]
        where the strict inequality follows because $t$ is linear, contains $x$ with positive coefficient and $z > b_{\max}$. Hence $I(a)\neq I(\subst{t}{x}{\repr{z}})$
        
      \item To prove that this element of $X_z$ cannot appear in any $\Gamma_{I,z'}(\phi)$ with $z' \prec z$, let   $f'(t')$ be an arbitrary, but fixed element of $\Gamma_{I,z'}(\phi)$ and prove that $f'(I(\subst{t'}{x}{\repr{z}}))\neq f(I(\subst{t}{x}{\repr{z}}))$. Again it suffices to analyze the case where $f$ and $f'$ are the same function symbols, and to prove that
      $I(\subst{t'}{x}{\repr{z}}))\neq I(\subst{t}{x}{\repr{z}})$. Since $t'$ and $t$ are arguments of the same function symbol, the respective coefficients of $x$ in $t$ and $t'$ are the same. Hence 
        \[
          I\models \subst{t'}{x}{z'}< \subst{t'}{x}{z}
        \]
        Moreover, $\Phi_{\text{max}}(\mathcal{S})$ ensures that $t$ is maximal among the terms of $f$ in $\mathcal{T}$, and hence
                \[
          I\models \subst{t'}{x}{z}< \subst{t}{x}{z}.
        \]
Hence $I(\subst{t'}{x}{\repr{z}})< I(\subst{t}{x}{\repr{z}}))$, which implies that the two sides of the inequality cannot be the same.
...
By the last conjunct in $\Psi_{\text{up}}(B,\mathcal{S})$, we have $I \models \subst{t'}{x}{z} \geq b_{\min}$.  
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        % \]
        \[
        I^*(a) \leq c_f \cdot b_{\max} + v_{\max}^f = c_f \cdot b_{\max} + I^*(\subst{t}{x}{0}) = I^*(\subst{t}{x}{\hat{b}_{\max}})
        \]
...
\begin{align*}
\Psi_{\text{up}}^\text{e}(B) &\equiv \Phi_\text{max}^\text{e} \land \Phi_\text{prop}^\text{e} \land \text{ClashUpEnc} \\
\Psi_{\text{down}}^\text{e}(B) &\equiv \Phi_\text{min}^\text{e} \land \Phi_\text{prop}^\text{e} \land \text{ClashDownEnc}
\end{align*}
where:
\begin{itemize}
    ...
    \item Upward propagation encoding $\Phi_{\text{prop}}^e$:
    \[
...
We have prepared a short test suite consisting of 16\marek{It would be better if I generated more
	problems where the algorithm succeeds but I don't know if it is necessary. The graphs do look
	a bit sparse though.} small problems such as
...
with increasing $c$. \textsc{z3} returns \textsc{unknown} for all but 4 problems starting
from $c = 2$. \textsc{cvc5} on the other hand fals back to solving just one problem
with $c \geq 4$. \textsc{z3} solved the 4 problems immediately regardless of $c$.
...
on all but one problem, which gets solved immediately.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\linewidth]{experiments/data/scaling_plot.png}
	\caption{Problems solved with increasing $c$}
	\label{fig:scaling}
\end{figure}
...
\end{align}
The clash condition\marek{add ref} requires $10000 \leq b_{\max} + 1$, therefore
...
We have presented a novel method for certifying satisfiability of universally quantified integer arithmetic formulas with uninterpreted function symbols—a fragment of UFLIA that arises naturally in verification tasks involving arrays and recursive constraints. Unlike traditional SMT solvers that rely on explicit model construction, our approach uses inductive propagation arguments to avoid the need for large or infinite models. By introducing \emph{satisfiability certificates} based on finite base intervals and propagation conditions, we reduce the problem of infinite-domain satisfiability to the verification of finite $\smt$ queries.

\paragraph{From Framework to Algorithm.}
Our approach unfolds in two complementary layers. First, we introduced a general \emph{certificate framework} (\cref{sec:certificate}) that captures the essence of inductive propagation: a certificate consists of a finite base assignment, a well-order on quantifier instantiations, and propagators that extend solutions step-by-step along that order. This framework is intentionally abstract—it works for an arbitrary number of quantified variables and any well-founded ordering, making it applicable beyond the fragment studied here. Its soundness (Property \ref{prop:certThenSat}) is established once and for all, independent of how certificates are found.

Second, we instantiated this framework with a concrete algorithm (\cref{sec:alg}) where the core of our approach is the \req condition (\cref{def:reqpivot-param}), which captures when solutions can be extended outward from a finite base interval $B$. We have shown that if $B$ satisfies base satisfiability and the propagation conditions hold, then the entire formula is satisfiable (\cref{thm:interval-certificate-param}). The algorithm derived from this insight (\cref{alg:interval-check-param-withBigvee}) is both sound and relatively complete: every \textsc{Sat} answer is correct, and it terminates with \textsc{Sat} for all satisfiable formulas satisfying the \req condition (\cref{thm:completeness-bigvee}). Moreover, by employing a compact CNF encoding of the propagation conditions, we avoid exponential blow-up and make the approach implementable within modern $\smt$ solvers.

\paragraph{Empirical Relevance.}
Something from Experiments section.

\paragraph{Limitations and Future Work.}
The current implementation assumes a single quantified variable with uniform coefficients per function. While some multi-variable formulas can be reduced via change of variables, generalizing the propagation conditions to arbitrary linear combinations remains open. Other natural extensions include developing algorithms that search for certificates with more complex propagation orders (e.g., lexicographic order) for multiple variables, generalizing the \req condition to handle non-collinear coefficients, nested functions, or more general arithmetic constraints.

\paragraph{Concluding Remarks.}
By shifting from model construction to inductive propagation, we have expanded the class of UFLIA formulas that can be decided practically. Our work bridges a gap between decidable fragments and heuristically solved cases, offering a certificate-based method for satisfiability in the presence of universal quantification, arithmetic, and uninterpreted functions. We believe this approach opens new avenues for verification tools that require both scalability and verifiable correctness.
