...
The key idea is to treat each quantifier as introducing infinitely many constraints on function calls, which we refer to as cells. A single cell may appear in multiple constraints, so we must
...
We focus on formulas that combine uninterpreted functions with linear integer arithmetic in a way that frequently appears in verification conditions, yet challenges existing decision procedures.

\paragraph{Formula Class}
Formally, we study formulas of the shape
...
\begin{itemize}
    ...
    \item For each $f_i$, the \emph{offset terms} $t^{f_i}_1,\dots,t^{f_i}_{l_i}$ are pairwise distinct ground terms of linear integer arithmetic,
    \item All occurrences of $f_i$ inside $Q$ are of the form $f_i(x+t)$ with $t$ being one of the offset terms; no nested applications of uninterpreted functions are allowed.
\end{itemize}
