...
\begin{enumerate}
    \item \textbf{Base formula}: $J \models F \land \bigwedge_{x \in B} Q[x]$, since $\mathcal{M} \models \phi$ and $J$ agrees with $\mathcal{M}$ on all relevant cells.
    ...
    \begin{itemize}
        ...
        \[
        J(a)\cmpsign{\coeffsign{f}-}  c_f \cdot b_{\min} + v_{\min}^f = c_f \cdot b_{\min} + J(\subst{t}{x}{0}) = J(\subst{t}{x}{\repr{b}_{\min}}),
        \]
