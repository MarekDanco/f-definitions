...
The algorithm presented so far relies on large disjunctions over all possible subset choices $\mathcal{S}\subseteq\mathcal{T}$ and $\mathcal{S}'\subseteq\mathcal{T}$. While theoretically elegant, these disjunctions lead to formulas exponentially growing with the size of the set $\mathcal{T}$, making them computationally impractical. In this subsection, we present a compact CNF encoding that avoids this issue.
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        \Bigl( (p_{f(t)} \land p_{f(t')}) \Rightarrow t = t' \;\land\; 
        (p_{f(t)} \land \neg p_{f(t')}) \Rightarrow t' \;\cmpsignstrict{\coeffsign{f}s}\; t \Bigr)}{x}{0}
    \end{align*}
   
    \item Propagation encoding $\Phi_{\text{prop}}^{\text{e}}$:%
    \[
    \bigwedge_{\mathcal{T}_{\!-}\subseteq\mathcal{T}}
    \left(\bigwedge_{f(t)\in\mathcal{T}_{\!-}} \neg p_{f(t)} \;\wedge\; \bigwedge_{f(t)\in\mathcal{T}\setminus\mathcal{T}_{\!-}} p_{f(t)}\;\Rightarrow\; \forall x\, \forall \bar{u}\, \exists \bar{v}\; Q^{\mathcal{T}_{\!-}}\right),
    \]
    ...
    \item Clash condition encoding $\text{ClashEnc}^s$:
      % \begin{align*}
\[  \bigwedge_{f(t)\in\mathcal {T}}\bigwedge_{a \in \args(F,f)} \left(p_{f(t)} \Rightarrow a\cmpsignstrict{\coeffsign{f}s}  \subst{t}{x}{\ivbound{B}{s}}\right)    \]
...
Note that if for some $f\in\mathcal{{F}}$ the ground parts of its argument terms (obtained by substituting $x=0$) are pairwise different, then at most one $p_{f(t)}$ can be true in any satisfying assignment of $\Psi^{s,\text{e}}(B)$. This follows from the extremal condition, which would otherwise require two distinct ground terms to be equal.
