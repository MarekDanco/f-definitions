...
\[
v_{\max}^f = \max\{\, I^*(t_0) \mid f(t) \in \mathcal{T}\,\}, \qquad
v_{\min}^f = \min\{\, I^*(t_0) \mid f(t) \in \mathcal{T} \,\}.
\]
where $t_0=\subst{t}{x}{0}$ for each $f(t)\in\mathcal{T}$. The conditions $\Phi_{\text{max}}(\mathcal{S}^*)$ and $\Phi_{\text{min}}(\mathcal{S}'^*)$ ensure that:
\begin{itemize}
    \item For all $f(t) \in \mathcal{S}^*$: $I^*(t_0) = v_{\max}^f$,
    \item For all $f(t) \in \mathcal{S}'^*$: $I^*(t_0) = v_{\min}^f$.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For each $f(t)\in\mathcal{S}^*$ and $a \in \args(F,f)$:\\ Since $I^*(t_0)=v_{\max}^f$, we have:
        % \[
        % I^*(t_g) \leq b_{\max} + v_{\max}^f = b_{\max} + I^*(t)
        % \]
        \[
        I^*(a) \leq c_f \cdot b_{\max} + v_{\max}^f = c_f \cdot b_{\max} + I^*(t_0) = I^*(\subst{t}{x}{b_{\max}})
        \]
    ...
    \begin{itemize}
        ...
        % \]
        \[
        I^*(a) \geq c_f \cdot b_{\min} + v_{\min}^f = c_f \cdot b_{\min} + I^*(t_0) = I^*(\subst{t}{x}{b_{\min}})
        \]
