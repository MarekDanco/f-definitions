...
\title{%
    Inductive Function Definitions\todoSN{We will change this.}
}
...
\begin{itemize}
    \item We formalise the notion of \emph{cell propagation certificates} (\cref{def:certificate}) that witness satisfiability through inductive extension of finite base assignments.
    \item We introduce the \emph{multi-\req condition} (\cref{def:multi-reqpivot-param}), propagation conditions that allows multiple functions to serve as pivots simultaneously.
    \item We present an interval-based algorithm (\cref{alg:interval-check-param-withBigvee}) that searches for suitable base intervals and checks propagation conditions using $\smt$ queries.
    \item We prove soundness (\cref{cor:soundness-bigvee}) and relative completeness (\cref{thm:completeness-bigvee}) for our algorithm.: it returns \textsc{Sat} for all satisfiable formulas meeting the multi-\req condition, and \textsc{Unsat} for formulas whose unsatisfiability can be detected through \emph{finite instantiation}.
    \item We show\todoSN{From the experimental part, hopefully?} experimentally that our method can handle problems beyond the reach of current SyGuS and MBQI approaches, often producing more interpretable models.
\end{itemize}
...
%    \item for each $f_i$, the \emph{offset terms} $t^{f_i}_1,\dots,t^{f_i}_{l_i}$ are pairwise distinct LIA ground terms, and
    \item all uninterpreted function symbols have maximal arity $1$\todoSR{for now}
...
\paragraph{Why is this Hard?}\todoSN{Not sure about this part! I'm just trying to summarise it from what's written in the Intro and Related Work sections.}
...
\begin{itemize}
    ...
    \item \textbf{Specialised array decision procedures} either forbid arithmetic on indices or are limited to difference‑bounded constraints.
\end{itemize}
...
%     \item We define the \emph{multi‑\req condition} (Definition~\ref{def:multi-reqpivot-param}) that characterises when propagation is possible.
...
\begin{definition}
	...
	Given a formula $\phi$ of the form $F\wedge \forall x\;.\; Q$, we call a cell interpretation $I$ such that
        ...
        \end{itemize}
 a \emph{pre-satisfiability certificate} of $\phi$. For any $z\in\mathbb{Z}$, we denote by $\Gamma_{I,z}(\phi)$ the resulting set of cells \[\{ I(\subst{t}{x}{\repr{z}}) \mid t\in args(Q, f), f \emph{ is an uninterpreted function symbol}\}.\]
\end{definition}
...
\section{Algorithm}\label{sec:alg}\todoSN{I changed the intro of this section into this one. What do you think?}
...
\cref{sec:problem-statement}, we now present our decision procedure for formulas of the form~\eqref{eq:main-fragment}. The core idea is to search for a finite interval $B = [b_{\min}, b_{\max}]$ from which a model can be inductively extended to all integers. This requires two components:
\begin{enumerate}
...
Our algorithm relies on the ability to propagate solutions outward from a base interval. We first formalise the propagation condition as the \emph{multi-\req condition}, then show how to combine it with interval expansion to obtain a complete decision procedure. 
