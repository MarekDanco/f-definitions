...
\testlength{
\begin{proof}
Assume $\phi$ is satisfiable and satisfies \req condition for $(\mathcal{S}^*,\mathcal{S}'^*)$. Let $\mathcal{J}$ be an interpretation with $\mathcal{J}\models\phi$. For each function $f \in \mathcal{F}$, let $c_f$ be the common coefficient of $x$ in all its arguments. For each $f(t)\in\mathcal{F}$, define:
\[
