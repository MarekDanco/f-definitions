...
\begin{itemize}
    ...
    \item Clash condition encoding $\text{ClashEnc}^s$:
      % \begin{align*}
\[  \bigwedge_{f(t)\in\mathcal {T}}\bigwedge_{a \in \args(F,f_i)} \big(s_{f(t)} \Rightarrow \bigwedge_{\substack{a \in \args(F,f)}} a\cmpsignstrict{\coeffsign{f}s}  \subst{t}{x}{\ivbound{B}{s}}\big)    \]
