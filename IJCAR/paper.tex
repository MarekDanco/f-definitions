...
\begin{itemize}
    ...
    \]
    where $\bar{u}=(u_{f(t)})_{f(t)\in\mathcal{T}_{-}}$ and $\bar{v}=(v_{f(t)})_{f(t)\in\mathcal{T}\setminus\mathcal{T}_{-}}$, and where $Q^{\mathcal{T}_{-}}$ is obtained from $Q$ by simultaneously replacing each $f(t)\in\mathcal{T}_{-}$ by $u_{f(t)}$ and each $f(t)\in\mathcal{T}\setminus\mathcal{T}_{-}$ by $v_{f(t)}$.
