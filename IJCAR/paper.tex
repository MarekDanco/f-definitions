...
%    \item for each $f_i$, the \emph{offset terms} $t^{f_i}_1,\dots,t^{f_i}_{l_i}$ are pairwise distinct LIA ground terms, and
    ...
    \item The coefficient of the quantified variable is the same for all argument terms of the same uninterpreted function symbol.\todoSR{I removed the restriction on the sign of the coefficient}
    \end{itemize}
...
added notation:\todoSR{I will move this to the notation section.}
\begin{itemize}
  
\item signs $+,-$
\item $\coeffsign{f}$: the sign of the common coefficients of $x$ in $\phi$
\item for any two signs $s$ and $s'$, $ss'$ is $+$ iff $s=s'$, and $-$, otherwise
  
\item for two integers $z, z'$, $z\cmpsignstrict{+} z'$ iff $z<z'$ and $z\cmpsignstrict{-} z'$ iff $z>z'$
\item for an interval $B=[b_{\min}, b_{\max}]$, $\ivbound{B}{-}= b_{\min}$, $\ivbound{B}{+}= b_{\max}$
\end{itemize}
...
\begin{itemize}
\item \textbf{Maximality condition $\Phi^s_{\text{max}}(\mathcal{S})$:}
  \vspace*{-0.2cm}
  \[
    ...
    \item \textbf{Propagability condition $\Phi_{\text{prop}}(\mathcal{S})$:}
        ...
        \end{itemize}
\end{itemize}
We say that $Q$ satisfies the \emph{\req condition} for subsets $\mathcal{S}\subseteq\mathcal{T}$ and $\mathcal{S}'\subseteq\mathcal{T}$ iff $\Phi^+_{\text{max}}(\mathcal{S})$, $\Phi_{\text{prop}}(\mathcal{S})$,
$\Phi^{-}_{\text{max}}(\mathcal{S}')$, and $\Phi_{\text{prop}}(\mathcal{S}')$ hold. Moreover we say that $Q$ satisfies the \emph{\req condition} iff there exists a pair~$(\mathcal{S}, \mathcal{S}')$ such that $Q$ satisfies the \req condition for $(\mathcal{S}, \mathcal{S}')$.
\end{definition}
...
For an interval $B\subseteq\mathbb{Z}$  a sign~$s\in\{-,+\}$, and a subset $\mathcal{S} \subseteq \mathcal{T}$, define
 $\Psi^{s}(B,\mathcal{S})$:=
    \[
    \Phi^{s}_{\text{max}}(\mathcal{S})\;\land\; 
    \Phi_{\text{prop}}(\mathcal{S}) \;\land
    \bigwedge_{f(t) \in \mathcal{S}} \bigwedge_{\substack{a \in \args(F,f)}} a\cmpsignstrict{s\coeffsign{f}}  \subst{t}{x}{\ivbound{B}{s}} 
    \]
    % \vspace*{-0.2cm}
    % \[\land 
...
\begin{itemize}
    ...
    \item To prove that this element of $X_z$ cannot be in $\mathit{def}(I)$, let $f'(t')$ be an arbitrary, but fixed element of $\mathit{def}(I)$. We prove that $f'(I(\subst{t'}{x}{\repr{z}}))\neq f(I(\subst{t}{x}{\repr{z}}))$. If $f$ is a different function symbol than $f'$ this is certainly the case, and hence it suffices to prove
      ...
      The clash condition $a\cmpsignstrict{\coeffsign{f}}\subst{t}{x}{\ivbound{B}{s}}$ in $\Psi^+(B,\mathcal{S})$ guarantees that for every ground argument $a \in \args(F,f)$:\todoSR{missing: $a\in \text{def}(I)$ but $a\not\in args(F,f)$}
        \[
      ...
      \item To prove that this element of $X_z$ cannot appear in any $\Gamma_{I,z'}(\phi)$ with $z' \prec z$, let $f'$ be an uninterpreted function symbol and $t'\in args(Q,f)$  and prove that $f'(I(\subst{t'}{x}{\repr{z}'}))\neq f(I(\subst{t}{x}{\repr{z}}))$. Again it suffices to analyze the case where $f$ and $f'$ are the same function symbols, and to prove that
        ...
        \]
        Moreover, $\Phi_{\text{max}}(\mathcal{S})$ ensures 
                \[
          I\models \subst{t'}{x}{\hat{z}}\cmpsignstrict{\coeffsign{f}}\subst{t}{x}{\hat{z}}.
        \]
...
\While{$F \land \bigwedge_{x=b_{\min}}^{b_{\max}} Q[x]$ is satisfiable}
    \State $\text{max}_\text{up}\gets\bigvee_{\mathcal{S}\subseteq \mathcal{T}}\Psi^+([b_{\min},b_{\max}],\mathcal{S}))$
    \State $\text{min}_\text{down}\gets\bigvee_{{\mathcal{S}'\subseteq \mathcal{T}}}\Psi^-([b_{\min},b_{\max}],\mathcal{S}))$ 
    \If{$\left(F \land \bigwedge_{x=b_{\min}}^{b_{\max}} Q[x]\right) \land \text{max}^+ \land \text{min}_{\text{down}}$ is satisfiable}
...
\begin{itemize}
    ...
    \item Clash condition encoding $\text{ClashEnc}^s$:
      % \begin{align*}
\[  \bigwedge_{f(t)\in\mathcal {T}}\bigwedge_{a \in \args(F,f_i)} \big(s_{f(t)} \Rightarrow \bigwedge_{\substack{a \in \args(F,f)}} a\cmpsignstrict{s\coeffsign{f}}  \subst{t}{x}{\ivbound{B}{s}}\big)    \]
...
Note that if for some $f\in\mathcal{{F}}$ the ground parts of the argument terms (obtained by substituting $x=0$) are pairwise different, then at most one $s_{f(t)}$ can be true in any satisfying assignment of $\Psi^{+,\text{e}}(B)$ or $\Psi^{-,\text{e}}(B)$. This is because the maximality condition would require two different ground parts to be  equal, which is impossible in this case. 

\begin{lemma}[Encoding Correctness]
\label{lem:encoding-correctness}
For any interval $B = [b_{\min}, b_{\max}]$,  and sign~$s\in\{+, -\}$, 
 $\Psi_\text{up}^{s,\text{e}}(B)$ is satisfiable iff there exists $\mathcal{S}\subseteq\mathcal{T}$ such that $\Psi_{\text{up}}(B,\mathcal{S})$ holds.
\end{lemma}
% \begin{proof}
