...
Instead of enumerating all possible subset choices, we introduce auxiliary Boolean variables that represent membership in the subsets $\mathcal{S}$ and $\mathcal{S}'$. For each term $f(t)\in\mathcal{T}$, we create:
\begin{itemize}
    \item $p_{f(t)}$: true iff $f(t) \in \mathcal{S}$ (for propagation with $s=+$)
    \item $p'_{f(t)}$: true iff $f(t) \in \mathcal{S}'$ (for propagation with $s=-$)
\end{itemize}
