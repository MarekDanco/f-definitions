...
Symmetrically, for downward propagation, define the minimality condition $\Phi_{\text{min}}$ with $<$ instead of $>$.
    ...
    \item \textbf{Propagability condition $\Phi_{\text{prop}}(\mathcal{S})$:}
        ...
        \end{itemize}
\end{itemize}
We say $Q$ satisfies the \emph{\req condition} for $\mathcal{S}\subseteq\mathcal{T}$ and $\mathcal{S}'\subseteq\mathcal{T}$ iff $\Phi_{\text{max}}(\mathcal{S})$, $\Phi_{\text{prop}}(\mathcal{S})$,
$\Phi_{\text{min}}(\mathcal{S}')$, and $\Phi_{\text{prop}}(\mathcal{S}')$ all hold. Moreover, $Q$ satisfies the \emph{\req condition} iff such a pair~$(\mathcal{S}, \mathcal{S}')$ exists.
\end{definition}
...
\forall x.\ f(x+c+3) + f(x+4) + f(x+1) = g(x+2) + h(x)
\]
where $c$ is a ground constant. Let $\mathcal{T} = \{f(x+c+3), f(x+4), f(x+1), g(x+2), h(x)\}$.

\medskip\noindent\textbf{Upward propagation with $\mathcal{S} = \{f(x+4),g(x+2)\}$:}
\begin{itemize}
    \item \textbf{Maximality}: For $f$, we require $(x+4) > (x+c+3)$ and $(x+4) > (x+1)$; for $g$, no constraints. With $x=0$: $4 > c+3$ and $4 > 1$, i.e., $c < 1$.
    \item \textbf{Propagability}: After substitution, we get:
    \[
    \begin{aligned}
        \forall x\;\forall u_{f(x+c+3)},\, u_{f(x+1)},\, u_{h(x)}\;\exists v_{f(x+4)},\, v_{g(x+2)}\; \text{such that}\\[4pt]
        u_{f(x+c+3)} + v_{f(x+4)} + u_{f(x+1)} = v_{g(x+2)} + u_{h(x)}.
    \end{aligned}
    \]
    This holds by taking $v_{g(x+2)} = 0$ and $v_{f(x+4)} = u_{h(x)} - u_{f(x+c+3)} - u_{f(x+1)}$.
\end{itemize}

\medskip\noindent\textbf{Downward propagation with $\mathcal{S}' = \{f(x+1),h(x)\}$:}
\begin{itemize}
    \item \textbf{Minimality}: For $f$, we require $(x+1) < (x+c+3)$ and $(x+1) < (x+4)$; for $h$, no constraints. With $x=0$: $1 < c+3$ and $1 < 4$, i.e., $c > -2$.
    \item \textbf{Propagability}: After substitution:
    \[
    \begin{aligned}
        \forall x\;\forall u_{f(x+c+3)},\, u_{f(x+4)},\, u_{g(x+2)}\;\exists v'_{f(x+1)},\, v'_{h(x)}\; \text{such that}\\[4pt]
        u_{f(x+c+3)} + u_{f(x+4)} + v'_{f(x+1)} = u_{g(x+2)} + v'_{h(x)}.
    \end{aligned}
    \]
    This holds by taking $v'_{h(x)} = 0$ and $v'_{f(x+1)} = u_{g(x+2)} - u_{f(x+c+3)} - u_{f(x+4)}$.
\end{itemize}

\medskip\noindent\textbf{Conclusion:}
For any $c \in (-2, 1)$, the pair $(\mathcal{S},\mathcal{S}')$ witnesses the \req condition.
\end{example}
...
\[
  ...
  \Biggl(\bigvee_{\mathcal{S}\subseteq \mathcal{T}}\Psi_{\text{up}}(B,\mathcal{S})\Biggr)\;\land\;
  \Biggl(\bigvee_{\mathcal{S}'\subseteq \mathcal{T}}\Psi_{\text{down}}(B,\mathcal{S}')\Biggr)
\]
is satisfiable, then $\phi$ has a satisfiability certificate, and hence is satisfiable.
\end{theorem}

\begin{proof}
  Suppose the conjunction is satisfiable, let $I^*$ be an interpretation satisfying it, and let $\mathcal{S}$ be such that $I^*\models \Psi_{\text{up}}(B,\mathcal{S})$, and $\mathcal{S}'$ such that $I^*\models \Psi_{\text{down}}(B,\mathcal{S}')$. Assume that  $B = [b_{\min}, b_{\max}]$. We construct a certificate as in \cref{def:certificate}:
...
Hence $I(\subst{t'}{x}{\repr{z}'})< I(\subst{t}{x}{\repr{z}}))$, which implies that the two sides of the inequality cannot be the same.
  ...
  \item The case $z < b_{\min}$ is symmetric, using the downward clash condition $a \geq \subst{t}{x}{\repr{b}_{\min}}$ and $\Phi_{\text{min}}(\mathcal{S}')$. Here, we prove that for all $z'\preceq z$, $I(\subst{t'}{x}{\repr{z}'})> I(\subst{t}{x}{\repr{z}}))$.
    Note that the fact that the upward propagation block of elements greater than $b_{\max}$ occurs before the downward propagation block of elements less than $b_{\min}$ in the order $\preceq$ does not create a problem with overlapping propagation, since $z'\preceq z$ with $z'>b_{\max}$ implies $z'>b_{\max}>b_{\min}>z$, which allows symmetric application of the argument of the case where $z>b_{\max}$.
