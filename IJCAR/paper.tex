...
iterations in \cref{alg:interval-check-param-withBigvee}.

We implemented our algorithm using the \textsc{z3} Python API. Our implementation only checks upward induction using~$\Psi^+$,
...
To study the behavior of direct application of $\smt$ solvers to our problems, and the influence
of the parameter~$c$ on their runtime, we ran our experiments with a 15-minute time limit.
...
\textsc{cvc5} in \textsc{sygus} mode solved all except two of our problems regardless
of the asserted bounds.
The first problem is
\begin{align*}
	f(0) = 1 \;\land \:\forall x \:. \:f(x+1) = 2*f(x).
\end{align*}
The second one is similar to the previous one with added integer constant in the quantified part.
We assume the reason for this is that the function $\lambda x. \:2^{x}$ is not in \textsc{cvc5}
default \textsc{sygus} grammar.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\linewidth]{experiments/data/scaling_plot.png}
	\caption{Problems solved with increasing $c$, with $*$ referring to unbounded problems.}
	\label{fig:scaling}
\end{figure}
