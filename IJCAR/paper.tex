...
\begin{definition}[Cell]\label{def:cell}
  ...
  \mj{sounds like we need to assume that for every $n\in\domain$, there exists
  an interpreted constant $n$. Might be nitpicky, but I'd make it explicit if
the domain is aribtrary (for $\mathbb{Z}$ it's kinda standard.}
\end{definition}
We denote the set of cells by $C(\domain)$.\marek{this means all cells that exist in the signature and also by plugging in all possible $n$-tuples from $\domain^n$?}\todoSN{Yes.}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item for all $z\in\domain^{\numvar{\bar{x}}}$, for every uninterpreted function symbol~$f$ and its argument terms $t\in args(Q, f)$, $I(\subst{t}{\bar{x}}{\repr{z}})\neq\bot$
			\marek{Is the domain of a cell interpretation really limited to be a set of cells?
			Maybe this should be $I(\subst{f(t)}{\bar{x}}{\repr{z}})\neq\bot$. Do I understand correctly that we place no requirement on $t$ of the form $x + c$?}\todoSN{Hmm I think the domain of $I$ is indeed a set of cells (Def 2). However, we extend $I$ to evaluate arbitrary ground terms via the unique compatible interpretation mentioned after Def 2.}
	\end{itemize}
