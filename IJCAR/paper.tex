...
There has been recent efforts in TPTP~\cite{SteenSFM23} and SMT2~\cite{Barbosa:23,BarbosaROTB19}
...
We introduce a method for certifying satisfiability of $\smt$ formulas with uninterpreted function symbols whose domain is a sort associated with a theory that has countable canonical model.\todoSR{I am not sure whether my usage of $\smt$ terminology is OK} In the case where this base theory is linear integer arithmetic, the resulting formulas belong to the 
...
We prepared a test suite consisting of 16 handcrafted satisfiable
...
We have presented a method for certifying satisfiability of formulas with universal quantifiers and uninterpreted function symbols, and a corresponding algorithm for a specific class of formulas in the SMTLIB theory UFLIA. By relying on induction, the method is able to succeed for formulas that do not have a finite model, and for formulas that are satisfiable only by functions that do not have a closed form representation. In addition to current syntactic restrictions, the introduced algorithm is restricted to satisfying the semantic $\req$ condition that enables induction. In future work, we intend to weaken this restriction, for example, using stronger forms of induction. Our long term vision is a general algorithm that is parametric in the used base theory and that only depends on a predicate defining the order in which to apply induction.
