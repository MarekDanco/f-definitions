...
\begin{enumerate}
    ...
    \item \textbf{Propagability $\Phi_{\text{prop-up}}(\mathcal{S})$:}
        ...
        \]
        This universally quantified formula is valid: for any values of $u_{f(x+c+3)}, u_{f(x+1)}, u_{h(x)}$, we can choose $v_{g(x+2)} = 0$ and $v_{f(x+4)} = u_{h(x)} - u_{f(x+c+3)} - u_{f(x+1)}$.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Propagability $\Phi_{\text{prop-down}}(\mathcal{S}')$:}
        ...
        \]
        This is also valid: choose $v'_{h(x)} = 0$ and $v'_{f(x+1)} = u_{g(x+2)} - u_{f(x+c+3)} - u_{f(x+4)}$.
\end{enumerate}

\medskip\noindent\textbf{Putting it together:}
$Q$ satisfies the multi-\req condition for the pair $(\mathcal{S},\mathcal{S}')$ whenever $c$ satisfies both $c < 1$ (from upward maximality) and $c > -2$ (from downward minimality). Hence for any $c \in (-2, 1)$, the specific choices $\mathcal{S} = \{f(x+4),g(x+2)\}$ and $\mathcal{S}' = \{f(x+1),h(x)\}$ witness the multi‑\req condition.
\end{example}
...
For an interval $B = [b_{\min}, b_{\max}]$ and subsets $\mathcal{S},\mathcal{S}' \subseteq \mathcal{T}$, define: \todoSR{potential improvement (to be left for later): conjunction over only one element of $S_i$ (the maximality condition forces them to be equal)}
...
The satisfiability propagators required by \cref{def:certificate} can often be obtained through quantifier elimination. For a given $z \in \mathbb{Z}$ and set of propagated cells $X_z$, the existence of values for $X_z$ that satisfy $Q[z]$ given values for the other cells is an existentially quantified formula $\exists \{v_{f(t)} \mid f(t) \in \mathcal{S}\}.\ Q'[z]$ where $v_{f(t)}$ correspond to the cells in $X_z$ and $Q'[z]$ is $Q[z]$ with those cells replaced by the $v_{f(t)}$. Quantifier elimination techniques for LIA can produce witnesses or solving functions for the $v_{f(t)}$, which serve as the required propagators.  propagators can then be used in the recursive evaluation function $val$ defined in \cref{sec:certificate} to compute values of the function that a model assigns to an uninterpreted function symbol.
...
\begin{itemize}
    \item For all $f(t) \in \mathcal{S}^*$: $I^*(t) = v_{\max}^f$,
    \item For all $f(t) \in \mathcal{S}'^*$: $I^*(t) = v_{\min}^f$.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For each $f(t)\in\mathcal{S}^*$ and $t_g \in \args(F,f)$ with $f(t) \in \mathcal{S}$:
        \[
    ...
    \begin{itemize}
        ...
        \item For each $f(t)\in\mathcal{S}'^{*}$ and $t_g \in \args(F,f)$:
        \[
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        \bigwedge_{f(t),f(t')\in\mathcal{T}} 
        \left(
        \big(s_{f(t)} \land s_{f(t')} \Rightarrow t = t'\big) 
        \ \land\
        \big(s_{f(t)} \land \neg s_{f(t')} \Rightarrow t > t'\big)
        \right)\right]}{x}{0}
    \end{align*}
...
% \end{align*}
    ...
    \item Upward propagation encoding: Given Boolean variables $\{s_{f(t)}\}$ representing the choice $\mathcal{S} = \{f(t)\mid s_{f(t)} \text{ is true}\}$, we define:
    \begin{align*}
        \Phi_{\text{prop-up}}^\text{e}(\{s_{f(t)}\}) &\equiv \forall x \forall \{u_{f(t)} \mid f(t) \in \mathcal{T}, \neg s_{f(t)}\} \exists \{v_{f(t)} \mid f(t) \in \mathcal{T}, s_{f(t)}\}.\ Q' 
    \end{align*}
    where the formula $Q'$ is obtained from $Q$ by simultaneously substituting:
    \begin{itemize}
        \item $f(t)$ by $v_{f(t)}$ whenever $s_{f(t)}$ is true,
        \item $f(t)$ by $u_{f(t)}$ whenever $s_{f(t)}$ is false.
    \end{itemize}

    \item Downward propagation encoding: Symmetrically, for downward propagation with variables $\{s'_{f(t)}\}$ representing $\mathcal{S}'$, define $\Phi_{\text{prop-down}}^\text{e}(\{s'_{f(t)}\})$.
    
    \item Clash condition encoding:
    \begin{align*}
        \text{ClashUpEnc} &\equiv \bigwedge_{f(t)\in\mathcal
        {T}}\bigwedge_{a \in \args(F,f_i)} \big(s_{f(t)} \Rightarrow (a \leq \subst{t}{x}{b_{\max}})\big) \\
