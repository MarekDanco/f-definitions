...
\begin{definition}
	...
	Given a formula $\phi$ of the form $F\wedge \forall \bar{x}\;.\; Q$, we call a cell interpretation $I$ such that
        \begin{itemize}
        \item $I\models F$, and
        \item for all $z\in\mathbb{Z}^{\numvar{x}}$, for every uninterpreted function symbol~$f$ and its argument terms $t\in args(Q, f)$, $I(\subst{t}{\bar{x}}{\repr{z}})\neq\bot$
        \end{itemize}
 a \emph{pre-satisfiability certificate} of $\phi$. For any $z\in\mathbb{Z}$, we denote by $\Gamma_{I,z}(\phi)$ the resulting set of cells \[\{ f(I(\subst{t}{\bar{x}}{\repr{z}})) \mid t\in args(Q, f), f \emph{ is an uninterpreted function symbol}\}.\]
\end{definition}
...
In this section, we will introduce an algorithm that will check satisfiability of an input formula by computing a satisfiability certificate of a certain form. \todoSN{Add this?}While our certificate framework (~\cref{def:certificate}) supports formulas with arbitrary numbers of universally quantified variables ($\numvar{\bar{x}} \geq 1$), the algorithm presented here focuses on the case $\numvar{\bar{x}}=1$. This certificate will explicitly certify: (1) satisfiability of the quantifier-free part $F$, and (2) satisfiability over a finite interval $B = [b_{\min}, b_{\max}]$ of instantiations of the quantified part $Q$. For values outside $B$, it uses satisfiability propagators with an order that propagates upward from $b_{\max}+1$ and downward from $b_{\min}-1$.  We will show soundness of the algorithm and completeness relative to conditions that ensure that such propagation outward from a base interval $B$ is possible.

Throughout this section we assume that $\phi$ is a formula of the form $F \land \forall x.\; Q$, as defined in ~\cref{sec:problem-statement}. Let $\mathcal{F}$ be the set of uninterpreted function symbols in $Q$, and $\mathcal{T}$ the set of terms of the form $f(t)$ in $Q$. \todoSR{I am not sure about the elements where the coefficient of $x$ in $t$ is zero.}
...
\begin{itemize}
    ...
    \[
        \subst{\left[ \bigwedge_{f\in\mathcal{F}}\left(\bigwedge_{f(t),f(t') \in \mathcal{S}} t = t' \;\land\;
        \bigwedge_{f(t) \in \mathcal{S}, f(t') \in \mathcal{T}, f(t')\not\in \mathcal{S}} t > t'
        \right)\right]}{x}{0}.
    \]
    \item \textbf{Upward propagability condition $\Phi_{\text{prop-up}}(\mathcal{S})$:}
        \[\forall x \forall \bar{u} \exists \bar{v}.\ Q'(\mathcal{S})\]
        where $\bar{u}$ represents the elements of $\mathcal{T}\setminus\mathcal{S}$ and $Q'$ is obtained from $Q$ by simultaneously replacing
        \begin{itemize}
            \item every element of $\mathcal{T}\setminus\mathcal{S}$ by $u$, and          
            \item every element of $\mathcal{S}$ by $v$.
        \end{itemize}
\end{itemize}
...
For an interval $B = [b_{\min}, b_{\max}]$ and subsets $\mathcal{S} \subseteq \mathcal{T}$, $\mathcal{S}'\subseteq \mathcal{T}$, define: \todoSR{potential improvement (to be left for later): conjunction over only one element of $S_i$ (the maximality condition forces them to be equal)}
...
\begin{enumerate}
    ...
    \bigwedge_{f\in\mathcal{F}} \bigwedge_{\substack{a \in \args(F,f) \\ f(t) \in \mathcal{S}}} a \leq \subst{t}{x}{b_{\max}}
    \]
...
Thus, for any choice of subsets $\mathcal{S}$, $\Psi_{\text{up}}(B,\mathcal{S})$ (respectively, $\Psi_{\text{down}}(B,\mathcal{S}')$) asserts that $\mathcal{S}$ satisfies the upward (resp.\ $\mathcal{S}'$ satisfies the downward) conditions of multi‑\req \emph{and additionally} satisfies the clash condition for interval $B$. The clash condition $a \leq \subst{t}{x}{b_{\max}}$ ensures that for every $f(t)\in\mathcal{S}$, and ground argument $a$ of $f$ appearing in $F$, the propagated cell $f(\subst{t}{x}{\repr{z}})$ with $z > b_{\max}$ is different from the ground cell $f_i(a)$; similarly for downward propagation.

\begin{theorem}[Interval Satisfiability]
\label{thm:interval-certificate-param}
Let $B = [b_{\min}, b_{\max}]$ be a non‑empty integer interval, and let $\mathcal{S}$, $\mathcal{S}'$ be subsets of $\mathcal{T}$. If \[
\Bigl(F \land \bigwedge_{x \in B} Q[x]\Bigr) \;\land\; \Psi_{\text{up}}(B,\mathcal{S}) \;\land\; \Psi_{\text{down}}(B,\mathcal{S}')
\]
is satisfiable, then $\phi$ has a cell propagation certificate, and hence is satisfiable.\todoSR{a concrete checkable certificate would hence be $I, B, \mathcal{S}, \mathcal{S}'$}
\end{theorem}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For $z > b_{\max}$: the condition $\Phi_{\text{prop-up}}(\mathcal{S})$ provides, for any given values for all non‑propagated cells in $\Gamma_{I,z}(\phi)$, values $\bar{v}$ such that assigning these values to each corresponding cell in $X_z$ satisfies $Q[z]$.
...
The formula in the premise of \cref{thm:interval-certificate-param} belongs to Presburger arithmetic (i.e., SMTLIB LIA), and hence its satisfiability can be algorithmically checked. Moreover, the theorem immediately yields an algorithm that searches for the parameters $B$, $\mathcal{S}$ and $\mathcal{S}$ of this theorem. This can be seen in \cref{alg:interval-check-param-withBigvee}.
...
Since $I \models \text{max}_{\text{up}}$, there exists some $\mathcal{S}$ such that 
$I \models \Psi_{\text{up}}(B,\mathcal{S})$. Similarly, $I \models \text{min}_{\text{down}}$ implies there exists 
$\mathcal{S}'$ such that $I \models \Psi_{\text{down}}(B,\mathcal{S}')$. Thus $I$ satisfies:
\[
...
\[
v_{\max}^f = \max\{\, I^*(t) \mid f(t) \in \mathcal{T}\,\}, \qquad
v_{\min}^f = \min\{\, I^*(t) \mid f(t) \in \mathcal{T} \,\}.
\]

The conditions $\Phi_{\text{max}}(\mathcal{S}^*)$ and $\Phi_{\text{min}}(\mathcal{S}'^*)$ ensure that:
\begin{itemize}
    \item For all $f(t) \in S^*$: $I^*(t) = v_{\max}^f$,
    \item For all $f(t) \in S'^*$: $I^*(t) = v_{\min}^f$.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For each $f\in\mathcal{F}$ and $t_g \in \args(F,f)$ with $f(t) \in \mathcal{S}$:
        \[
...
The algorithm presented so far relies on large disjunctions over all possible subset choices $\mathcal{S}$ and $\mathcal{S}'$. While theoretically elegant, these disjunctions lead to formulas with exponential size, making them computationally impractical. In this subsection, we present a compact CNF encoding that avoids this issue.

\paragraph{Encoding Idea.} Instead of enumerating all subset choices we introduce auxiliary Boolean variables that represent membership of terms in $\mathcal{T}$ in these sets. For each term $f(t)\in\mathcal{T}$, we create:
\begin{itemize}
    \item $s_{f,t}$: true iff $f(t)\in \mathcal{S}$ (for upward propagation)
    \item $s'_{f,t}$: true iff $f(t)\in \mathcal{S}'_i$ (for downward propagation)
\end{itemize}
...
\begin{itemize}
    ...
    \begin{align*}
        \subst{\left[\bigwedge_{f\in\mathcal{F}} 
        \bigwedge_{f(t)\in \mathcal{T},f(t')\in\mathcal{T}} \big(s_{f,t} \land s_{f,t'} \Rightarrow t = t'\big) \ \land\
