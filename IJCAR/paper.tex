...
Our experiments will test the conjecture that the algorithm introduced in the previous two sections is able to improve upon state-of-the-art $\smt$ solvers concerning the capability of showing formulas in the considered class to be satisfiable.
...
We prepared a test suite of 28 problems\footnote{All the data for our experiments are available at our public repository \\\url{https://github.com/MarekDanco/f-definitions}}
such as
...
Apart from the upper bound $10^c$, our problems do not contain big integers, which would make a smarter initialization of the interval~$[b_{\min},b_{\max}]$ necessary in order to avoid a large number of loop iterations.\todoSR{I added a disclaimer about large integers already here. The issue is more subtle than this, but a discussion of this would probably lead too far.}

We implemented our algorithm using the \textsc{z3} Python API. Our implementation only checks upward induction using~$\Psi^+$, but leaves out downward induction, which is sound due to the guard described in the previous paragraph. We also applied the \textsc{smt} solvers \textsc{z3} and
\textsc{cvc5} directly to our problems, explicitly turning on model based quantifier instantiation~\cite{ge-cav09}.

It turned out that our algorithm terminates on all problems in negligible time. Moreover,  the value of the parameter~$c$ does not have a noticeable influence on runtime for the bounded versions of the test problems. We found the former observation surprising, while the latter was to be expected, due to the use of induction.

\todoSR{Maybe we should partially replace some of the text by two more lines in Figure~\ref{fig:scaling} corresponding to a timeout of 1 minute?}
For studying the behavior of direct application of $\smt$ solvers to our problems, and the influence of the parameter~$c$ on their runtime we ran our experiments with a 30-minute time limit.
...
solved 2.\todoSR{Can we say anything more about this? My guess is that CVC5 needs the model to be finite, and z3 needs it to be piecewise linear.}
\begin{figure}[ht]
	\centering
