...
Then, for the assignment $I_\phi$ from the proof of Property~\ref{prop:certThenSat}, which satisfies a given formula~$\phi$, and a given cell~$u$, the value of $I_\phi(u)$ can be computed by the following recursive function $val(u): C(\domain)\rightarrow \codomain$:
...
\[
v_{\max}^f = \max\{\, J(\subst{t}{x}{0}) \mid f(t) \in \mathcal{T}\,\}, \qquad
v_{\min}^f = \min\{\, J(\subst{t}{x}{0}) \mid f(t) \in \mathcal{T} \,\}.
\]
...
\begin{itemize}
    \item For all $f(t) \in \mathcal{S}^*$: $J(\subst{t}{x}{0}) = v_{\max}^f$, and
    \item For all $f(t) \in \mathcal{S}'^*$: $J(\subst{t}{x}{0}) = v_{\min}^f$.
\end{itemize}
Now choose
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For each $f(t)\in\mathcal{S}^*$ and $a \in \args(F,f)$:\\ By construction of $b_{\max}$ and since $J(\subst{t}{x}{0})=v_{\max}^f$, we have:   % \[
        % I^*(t_g) \leq b_{\max} + v_{\max}^f = b_{\max} + I^*(t)
        % \]
        \[
        J(a) \cmpsign{\coeffsign{f}+} c_f b_{\max} + v_{\max}^f = c_f  b_{\max} + J(\subst{t}{x}{0}) = J(\subst{t}{x}{\hat{b}_{\max}}),
        \]
    ...
    \begin{itemize}
        ...
        \[
        J(a)\cmpsign{\coeffsign{f}-}  c_f b_{\min} + v_{\min}^f = c_f b_{\min} + J(\subst{t}{x}{0}) = J(\subst{t}{x}{\repr{b}_{\min}}),
        \]
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        (p^s_{f(t)} \land \neg p^s_{f(t')}) \Rightarrow t' \;\cmpsignstrict{\coeffsign{f}s}\; t \Bigr)}{x}{0}
    \end{align*}
