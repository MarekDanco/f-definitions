...
\item proof soundness and relative\todoSR{I will make this more precise} completeness of this algorithm, and we
...
The sub-class of UFLIA formulas that the algorithm can handle\todoSR{We will update this as soon as we have fixed all aspects of this class.} allows one universal quantifier, and arguments of uninterpreted functions within quantified subformulas are restricted to terms of the form $x + t$, where $x$ is the universally quantified variable and $t$ is a ground term. 
...
Kunčak et al. \cite{kuncak-pldi10,kuncak-sttt13} introduces single-invocation synthesis. Hozzová et al. \cite{hozzova-ijcar24} synthesize recursive programs from constraints.\todoSR{Do we want to keep these references? If yes, we need a better description.}
...
\end{equation}
where $\forall x$ stands for universal quantification of arbitrarily many variables, $F$  and $Q$ are quantifier‑free formulas in the SMTLIB theory UFLIA of uninterpreted function symbols and linear integer arithmetic. Additionally:
\begin{itemize}
    \item All arguments of uninterpreted function symbols in $Q$ are LIA terms in which the universally quantified variable $x$ appears with positive coefficient.
    \item For each function symbol $f_i$, all its arguments in $Q$ have the \emph{same} coefficient of $x$.
\end{itemize}
We will define a way of certifying satisfiability of formulas in this class\todoSR{I am considering to generalize the certificate to the case of several universally quantified variables}\todoSR{We have no completeness result for such certificate}.

\begin{example}
  \[f(0)=0 \wedge \forall x \,.\, f(2x+3)=g(x)+f(2x+c+1),\]
  where $f$ has coefficient $2$ in all its arguments, $g$ has coefficient $1$, and $c$ is an uninterpreted constant. Both functions satisfy the same‑coefficient requirement.
\end{example}
\todoSR{What about decidability of this restricted class? I am currently working with the assumption that it is unknown}
...
Based on this, we will design an algorithm for detecting satisfiability under the additional assumptions that:
\begin{itemize}
     \item the number of universally quantified variables is $1$,
    \item all uninterpreted function symbols have arity $1$,
    \item all arguments of uninterpreted function symbols in $Q$ are of the form $x + \tau$ where $\tau$ is a ground term.
\end{itemize}
...
\begin{enumerate}
    ...
    \item All argument terms in $Q$ are linear in the quantified variable $x$ with a positive integer coefficient (the same for all arguments of a given function).
...
We already used the notation $\forall x$ as an abbreviation for universal quantification over several variables. In this case, we will write $\numvar{x}$ for the number of those variables. We will write $\subst{s}{v}{t}$ for the result of substituting the term $t$ for $v$ in the term or formula~$s$. Here, $v$ may again represent several variables, in which case we require $t$ to represent a $\numvar{v}$-vector of terms, and assume substitution to be parallel. For an integer $z\in\mathbb{Z}$, we will denote the constant representing $z$ in a logical formula by $\repr{z}$\todoSR{Does there exist some standard notation for this?}.
...
\begin{definition}
	...
	Given a formula $\phi$ of the form $F\wedge \forall x\;.\; Q$, we call a cell interpretation $I$ such that
        \begin{itemize}
        \item $I\models F$, and
        \item for all $z\in\mathbb{Z}^{\numvar{x}}$, for every uninterpreted function symbol~$f$ and its argument terms $t\in args(Q, f)$, $I(\subst{t}{x}{\repr{z}})\neq\bot$
        \end{itemize}
...
\begin{definition}
	...
	\begin{itemize}
		...
		\item a well-order $\preceq$ on $\mathbb{Z}^{\numvar{x}}$,%\footnote{A well-order total order for which every non-empty subset has a least element. This enables induction. If we want to allow sub-formulas to be at the same place in the order, we either need to combine them into equivalence classes, or relax anti-symmetry (i.e., work with a pre-wellorder).}
...
The notion of certificate defined here is abstract in the sense that it does not fix a concrete finite representation that allows for an independent check whether a given object is indeed a satisfiability certificate. For arriving at independently checkable certificates, it suffices to fix a well-order and decide on a representation of propagated cells and satisfiability propagator that allows for such an independent check. We refer the reader to the next section for a concrete way of doing so.\todoSR{I will try to make the connection clearer.}
...
In this section, we will introduce an algorithm that will check satisfiability of an input formula by computing a satisfiability certificate of a certain form. \todoSN{Add this?}While our certificate framework (~\cref{def:certificate}) supports formulas with arbitrary numbers of universally quantified variables ($\numvar{x} \geq 1$), the algorithm presented here focuses on the case $\numvar{x}=1$. This certificate will explicitly certify: (1) satisfiability of the quantifier-free part $F$, and (2) satisfiability over a finite interval $B = [b_{\min}, b_{\max}]$ of instantiations of the quantified part $Q$. For values outside $B$, it uses satisfiability propagators with an order that propagates upward from $b_{\max}+1$ and downward from $b_{\min}-1$.  We will show soundness of the algorithm and completeness relative to conditions that ensure that such propagation outward from a base interval $B$ is possible.

Throughout this section we assume that $\phi$ is a formula of the form $F \land \forall x.\; Q$, as defined in ~\cref{sec:problem-statement}. Let $f_1, \dots, f_r$ be the uninterpreted function symbols appearing in $Q$. For each $f_i$, let $T_i = \args(Q, f_i) = \{t^{f_i}_1, \dots, t^{f_i}_{l_i}\}$ be its argument terms in $Q$.

We will now formalize the condition ensuring propagation outward from a base interval $B$ is possible. For upward propagation, we select for each function $f_i$ a subset $S_i \subseteq T_i$ of its argument terms that are \emph{maximal} (all equal and strictly greater than other terms of $f_i$). The \emph{propagability condition} then guarantees that, no matter what values are assigned to the non‑maximal occurrences, we can find values $v_1,\dots,v_r$ such that assigning $v_i$ to all occurrences $f_i(t)$ with $t\in S_i$ makes $Q$ true. Different functions may receive different values. The downward condition is symmetric, selecting \emph{minimal} terms.
...
\begin{itemize}
    ...
    \[
        \subst{\left[ \bigwedge_{i=1}^{r} \left(\bigwedge_{t,t' \in S_i} t = t' \;\land\;
        \bigwedge_{t \in S_i, t' \in T_i \setminus S_i} t > t'
        \right)\right]}{x}{0}.
    \]
    \item \textbf{Upward propagability condition $\Phi_{\text{prop-up}}(\mathcal{S})$:}
        \[\forall x \forall \bar{u}_1 \dots \forall \bar{u}_r \exists v_1\dots \exists v_r.\ Q'(\mathcal{S})\]
        where $Q'(\mathcal{S})$ is obtained from $Q$ by simultaneously replacing:
        \begin{itemize}
            \item every $f_i(t)$ with $t \in S_i$ by $v_i$,
            \item every $f_i(t)$ with $t \in T_i \setminus S_i$ by $u_i^t$.
        \end{itemize}
        The variables $\bar{u}_i = \{u_i^t \mid t \in T_i \setminus S_i\}$ represent the non‑propagated cells.
\end{itemize}
...
\label{ex:multi-req-illustration}
Consider the formula:
\[
\forall x.\ f(x+c+3) + f(x+4) + f(x+1) = g(x+2) + h(x)
\]
where $c$ is a ground constant. All function arguments have the form $x + \tau$ where $\tau$ is ground. We extract the offset parts:
\begin{align*}
T_f &= \{x+c+3,\ x+4,\ x+1\}, \\
T_g &= \{x+2\}, \\
T_h &= \{x\}.
\end{align*}
Note that these are terms containing $x$, not just ground offsets.

\medskip\noindent\textbf{Upward propagation:}
Consider $\mathcal{S} = (S_f, S_g, S_h) = (\{x+4\}, \{x+2\}, \emptyset)$.
\begin{enumerate}
    \item \textbf{Maximality $\Phi_{\text{max}}(\mathcal{S})$:}
        \[
        \subst{\left[
        \begin{aligned}
        &\bigwedge_{t,t' \in S_f} t = t' \ \land\ 
        \bigwedge_{t \in S_f, t' \in T_f \setminus S_f} t > t' \\
        \land\ &\bigwedge_{t,t' \in S_g} t = t' \ \land\ 
        \bigwedge_{t \in S_g, t' \in T_g \setminus S_g} t > t' \\
        \land\ &\bigwedge_{t,t' \in S_h} t = t' \ \land\ 
        \bigwedge_{t \in S_h, t' \in T_h \setminus S_h} t > t'
        \end{aligned}
        \right]}{x}{0}
        \]
        Since $S_f = \{x+4\}$, the first conjunction $(x+4 = x+4)$ is trivial. 
        $T_f \setminus S_f = \{x+c+3, x+1\}$, so we get:
        \[
        (x+4 > x+c+3) \land (x+4 > x+1) \land (x+2 = x+2)
        \]
        Substituting $x=0$ gives:
        \[
        (4 > c+3) \land (4 > 1) \land (2 = 2)
        \]
    ...
    \item \textbf{Propagability $\Phi_{\text{prop-up}}(\mathcal{S})$:}
        In $Q$, replace:
        \begin{itemize}
            \item $f(x+4)$ by a fresh variable $v_f$,
            \item $g(x+2)$ by $v_g$,
            \item $f(x+c+3)$ by $u_f^{c+3}$,
            \item $f(x+1)$ by $u_f^1$,
            \item $h(x)$ by $u_h^0$.
        \end{itemize}
        ...
        \]
        This universally quantified formula is valid: for any values of $u_f^{c+3}, u_f^1, u_h^0$, we can choose $v_g = 0$ and $v_f = u_h^0 - u_f^{c+3} - u_f^1$.
\end{enumerate}

\medskip\noindent\textbf{Downward propagation:}
Consider $\mathcal{S}' = (S'_f, S'_g, S'_h) = (\{x+1\}, \emptyset, \{x\})$.
\begin{enumerate}
    \item \textbf{Minimality $\Phi_{\text{min}}(\mathcal{S}')$:}
        \[
        \subst{\left[
        \begin{aligned}
        &\bigwedge_{t,t' \in S'_f} t = t' \ \land\ 
        \bigwedge_{t \in S'_f, t' \in T_f \setminus S'_f} t < t' \\
        \land\ &\bigwedge_{t,t' \in S'_h} t = t' \ \land\ 
        \bigwedge_{t \in S'_h, t' \in T_h \setminus S'_h} t < t'
        \end{aligned}
        \right]}{x}{0}
        \]
        Since $S'_f = \{x+1\}$ and $T_f \setminus S'_f = \{x+c+3, x+4\}$, we get:
        \[
        (x+1 < x+c+3) \land (x+1 < x+4) \land (x = x)
        \]
        Substituting $x=0$ gives:
        \[
        (1 < c+3) \land (1 < 4) \land (0 = 0)
        \]
    ...
    \item \textbf{Propagability $\Phi_{\text{prop-down}}(\mathcal{S}')$:}
        ...
        \]
        This is also valid: choose $v'_h = 0$ and $v'_f = u_g^2 - u_f^{c+3} - u_f^4$.
\end{enumerate}

\medskip\noindent\textbf{Putting it together:}
$Q$ satisfies the multi-\req condition for the pair $(\mathcal{S},\mathcal{S}')$ whenever $c$ satisfies both $c < 1$ (from upward maximality) and $c > -2$ (from downward minimality). Hence for any $c \in (-2, 1)$, the specific choices $\mathcal{S} = (\{x+4\},\{x+2\},\emptyset)$ and $\mathcal{S}' = (\{x+1\},\emptyset,\{x\})$ witness the multi‑\req condition.
\end{example}
...
\begin{enumerate}
    ...
    \bigwedge_{i=1}^{r} \bigwedge_{\substack{a \in \args(F,f_i) \\ t \in S_i}} a \leq \subst{t}{x}{b_{\max}}
    \]
    where $\Phi_{\text{max}}(\mathcal{S})$ and $\Phi_{\text{prop-up}}(\mathcal{S})$ are as defined in \cref{def:multi-reqpivot-param}. \todoSR{The similarity of the names $t_g$ and $t$ seems to suggest that both are offsets, but $t_g$ is not. I might have a solution to this problem that does \emph{not} need renaming.}
    ...
    \bigwedge_{i=1}^{r} \bigwedge_{\substack{a \in \args(F,f_i) \\ t \in S_i'}} a \geq \subst{t}{x}{b_{\min}}
    \]
    where $\Phi_{\text{min}}(\mathcal{S}')$ and $\Phi_{\text{prop-down}}(\mathcal{S}')$ are as defined in \cref{def:multi-reqpivot-param}.
\end{enumerate}
\end{definition}

Thus, for any choice of subsets $\mathcal{S} = (S_1,\dots,S_r)$, $\Psi_{\text{up}}(B,\mathcal{S})$ (respectively, $\Psi_{\text{down}}(B,\mathcal{S}')$) asserts that $\mathcal{S}$ satisfies the upward (resp.\ $\mathcal{S}'$ satisfies the downward) conditions of multi‑\req \emph{and additionally} satisfies the clash condition for interval $B$. The clash condition $a \leq \subst{t}{x}{b_{\max}}$ ensures that for every ground argument $a$ of $f_i$ appearing in $F$, the propagated cell $f_i(\subst{t}{x}{\repr{z}})$ with $z > b_{\max}$ and $t \in S_i$ is different from the ground cell $f_i(a)$; similarly for downward propagation.

\begin{theorem}[Interval Satisfiability]
\label{thm:interval-certificate-param}
Let $B = [b_{\min}, b_{\max}]$ be a non‑empty integer interval, and let $\mathcal{S} = (S_1,\dots,S_r)$, $\mathcal{S}' = (S'_1,\dots,S'_r)$ be subsets of $T_i$ terms. If 
\[
\Bigl(F \land \bigwedge_{x \in B} Q[x]\Bigr) \;\land\; \Psi_{\text{up}}(B,\mathcal{S}) \;\land\; \Psi_{\text{down}}(B,\mathcal{S}')
\]
is satisfiable, then $\phi$ has a cell propagation certificate, and hence is satisfiable.
\end{theorem}
...
\begin{enumerate}
    ...
    \bigl\{ f_i(I(\subst{t}{x}{z})) \mid i=1,\dots,r,\; t \in S_i \bigr\} & \text{if } z > b_{\max}, \\[4pt]
    \bigl\{ f_i(I(\subst{t}{x}{z})) \mid i=1,\dots,r,\; t \in S_i' \bigr\} & \text{if } z < b_{\min}.
    \end{cases}
    \]
    Note that the maximality and minimality conditions ensure that for $z\not\in B$, $i\in 1,\dots,r$, $|\{ n \mid f_i(n)\in X_z\}|\leq 1$.

    \item \textbf{Satisfiability propagators:}
    \begin{itemize}
        \item For $z \in B$: $I$ itself satisfies $Q[z]$.
        \item For $z > b_{\max}$: the condition $\Phi_{\text{prop-up}}(\mathcal{S})$ provides, for any given values for all non‑propagated cells in $\Gamma_{I,z}(\phi)$, values $v_1,\dots,v_r$ such that assigning $v_i$ to each cell of function $f_i$ in $X_z$ satisfies $Q[z]$.
...
\begin{itemize}
    ...
    \item For $z > b_{\max}$, consider any $f_i(I(\subst{t}{x}{z}))$ with $t \in S_i$:
    \begin{itemize}
        \item It cannot belong to $\mathit{def}(I)$ because the clash condition $a \leq \subst{t}{x}{b_{\max}}$ in $\Psi_{\text{up}}(B,\mathcal{S})$ guarantees that for every ground argument $a \in \args(F,f_i)$:
        \[
        ...
        \begin{enumerate}
            \item If $z' \leq b_{\max}$: For any $t' \in T_i$, 
            \[
            I(\subst{t'}{x}{z'}) \leq I(\subst{t'}{x}{b_{\max}}) < I(\subst{t}{x}{z}),
            \]
            since $t$ is maximal among $T_i$ by $\Phi_{\text{max}}(\mathcal{S})$ and $z > b_{\max}$.
            
            \item If $b_{\max} < z' < z$: Then 
            \[
            I(\subst{t'}{x}{z'}) \leq \max_{t'' \in T_i} I(\subst{t''}{x}{z'}) < I(\subst{t}{x}{z})
            \]
...
\cref{thm:interval-certificate-param} immediately yields an algorithm that searches for an interval $B$ satisfying the three conditions. This can be seen in \cref{alg:interval-check-param-withBigvee}. It handles the case where all arguments have the form $x + \tau$ with $\tau$ ground. The general theory supports arguments with arbitrary positive coefficients.
...
The proof of~\cref{thm:interval-certificate-param} also shows how the return value \textsc{Sat} results in a corresponding satisfiability certificate, and extending the algorithm to also return such a certificate is an easy exercise.

The satisfiability propagators required by \cref{def:certificate} can often be obtained through quantifier elimination. For a given $z \in \mathbb{Z}$ and set of propagated cells $X_z$, the existence of values for $X_z$ that satisfy $Q[z]$ given values for the other cells is an existentially quantified formula $\exists v_1 \dots \exists v_k.\, Q'[z]$ where $v_1,\dots,v_k$ correspond to the cells in $X_z$ and $Q'[z]$ is $Q[z]$ with those cells replaced by the $v_i$. Quantifier elimination techniques for LIA can produce witnesses or solving functions for the $v_i$, which serve as the required propagators.
...
\paragraph{Encoding Idea.} Instead of enumerating all subset choices, we introduce auxiliary Boolean variables that represent membership in the subsets $S_i$ and $S'_i$. For each argument term $t_j^{f_i} \in T_i$, we create:
\begin{itemize}
...
where:
\begin{itemize}
    \item Maximality encoding $\Phi_\text{max}^\text{e}$:\todoSN{Another form is in the comment lines.}
    \begin{align*}
        \subst{\left[\bigwedge_{i=1}^r \left(
        \bigwedge_{j<k} \big(s_{i,j} \land s_{i,k} \Rightarrow t_j^{f_i} = t_k^{f_i}\big) \ \land\
        \bigwedge_{j,k} \big(s_{i,j} \land \neg s_{i,k} \Rightarrow t_j^{f_i} > t_k^{f_i}\big)\right)\right]}{x}{0}
    \end{align*}
...
% \begin{aligned}
% \bigwedge_{j<k}\,&
% s_{i,j} \land s_{i,k} &\Rightarrow&\ t_j^{f_i} = t_k^{f_i} \\
% \land\ \bigwedge_{j,k}\,&
% s_{i,j} \land \neg s_{i,k} &\Rightarrow&\ t_j^{f_i} > t_k^{f_i}
% \end{aligned}
% \right)
% \right]}{x}{0}
% \end{align*}
    ...
    \begin{itemize}
        \item $f_i(t_j^{f_i})$ by $v_i$ whenever $s_{i,j}$ is true,
        \item $f_i(t_j^{f_i})$ by $u_{i,j}$ whenever $s_{i,j}$ is false.
    \end{itemize}
    ...
    \begin{align*}
        \text{ClashUpEnc} &\equiv \bigwedge_{i=1}^r \bigwedge_{t_g \in \args(F,f_i)} \bigwedge_{j=1}^{l_i}\big(s_{i,j} \Rightarrow (t_g \leq \subst{t_j^{f_i}}{x}{b_{\max}})\big) \\
        \text{ClashDownEnc} &\equiv \bigwedge_{i=1}^r \bigwedge_{t_g \in \args(F,f_i)} \bigwedge_{j=1}^{l_i}\big(s'_{i,j} \Rightarrow (t_g \geq \subst{t_j^{f_i}}{x}{b_{\min}})\big)
    \end{align*}
\end{itemize}
\end{definition}

Note that if for some $i\in \{1,\dots, r\}$ the ground parts of the argument terms (obtained by substituting $x=0$) are pairwise different, then at most one $s_{i,j}$ can be true in any satisfying assignment of $\Psi_{\text{up}}^\text{e}(B)$ or $\Psi_{\text{down}}^\text{e}(B)$. This is because the maximality condition would require two different ground parts to be both equal and ordered, which is impossible. Similarly for minimality condition.
...
\end{lemma}
\begin{proof}
    ($\Rightarrow$) Suppose $\Psi_\text{up}^\text{e}(B)$ is satisfiable by an assignment $I$. Define $S_i = \{t_j^{f_i} \mid I(s_{i,j}) = \text{true}\}$. By $\Phi_{\text{max}}^\text{e}$, all $t \in S_i$ have equal ground parts under $I$, and they are strictly greater than those of any $t' \in T_i \setminus S_i$. Since all terms have the same coefficient of $x$, this implies, this implies $\Phi_{\text{max}}(\mathcal{S})$ holds. By $\Phi_{\text{prop-up}}^\text{e}$, all $f_i(t)$ with $t \in S_i$ are equal to $v_i$, satisfying the substitution in $\Phi_{\text{prop-up}}(\mathcal{S})$. Finally, $\text{ClashUpEnc}$ ensures the clash condition $t_g \leq \subst{t}{x}{b_{\max}}$ for all $t \in S_i$. Hence $\Psi_{\text{up}}(B,\mathcal{S})$ holds.

    ($\Leftarrow$) Suppose $\Psi_{\text{up}}(B,\mathcal{S})$ holds for some $\mathcal{S}$. Set $s_{i,j}$ true iff $t_j^{f_i} \in S_i$. By $\Phi_{\text{max}}(\mathcal{S})$, all selected terms have equal ground parts (after $x=0$ substitution) and these are greater than non-selected ones, so $\Phi_{\text{max}}^\text{e}$ holds. Let $v_i$ be the common value for $f_i(t)$ with $t \in S_i$ (guaranteed by $\Phi_{\text{prop-up}}(\mathcal{S})$), then $\Phi_{\text{prop-up}}^\text{e}$ holds. The clash condition from $\Psi_{\text{up}}(B,\mathcal{S})$ ensures $\text{ClashUpEnc}$. Thus $\Psi_\text{up}^\text{e}(B)$ is satisfiable.
