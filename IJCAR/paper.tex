...
%    \item for each $f_i$, the \emph{offset terms} $t^{f_i}_1,\dots,t^{f_i}_{l_i}$ are pairwise distinct LIA ground terms, and
    \item all uninterpreted function symbols have maximal arity $1$\todoSR{Section~\label{sec:certificate} does not require this. It would also be not too difficult to generalize that section to several quantified variables.}
