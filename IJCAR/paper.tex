...
% However, we require that enough cells are defined to (i)~check satisfaction of quantifier-free parts of formulas, and (ii)~determine the value of all arguments of uninterpreted function symbols for every universally quantified subformula and every instantiation of the universal quantifier.\mj{suggested rewrite, still bit too long}

In order to enable computer representation of cell interpretations, we work with finitely defined cell interpretations that are nevertheless sufficiently expressive to (i)~check satisfaction of quantifier-free parts of formulas, and (ii)~determine the value of all arguments of uninterpreted function symbols for every universally quantified subformula and every instantiation of the universal quantifier.\mj{suggested rewrite, still bit too long}\todoSN{Like this? Previous version is commented-out.}
...
\end{equation}
For each problem, we also used bounded versions of the form $F\wedge \forall x \:.\: (0\leq x \leq 10^c\Rightarrow Q)$, with $c\in \{1,\dots,5\}$.

We implemented our algorithm using the \textsc{z3} Python API. We also applied the \textsc{smt} solvers \textsc{z3} and
...
problems such as\todoSR{can we quantify what ``struggles'' roughly means?}
\begin{align*}
