...
\[\Psi^{s,\text{e}}(B) \equiv \Phi^{s,\text{e}}_\text{max} \land \Phi_\text{prop}^\text{e} \land \text{ClashEnc}\]
...
\begin{itemize}
    ...
    \item Propagation encoding $\Phi_{\text{prop}}^e$:
    \[
...
%    \begin{align*}
\[        \text{ClashEnc} \equiv \bigwedge_{f(t)\in\mathcal {T}}\bigwedge_{a \in \args(F,f_i)} \big(s_{f(t)} \Rightarrow (a \leq \subst{t}{x}{b_{\max}})\big)    \]
