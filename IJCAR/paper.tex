...
Instead of enumerating all possible subset choices, we introduce auxiliary Boolean variables that represent membership in the subsets $\mathcal{S}$ and $\mathcal{S}'$. Reducing our presentation to just one of the two cases, for each term $f(t)\in\mathcal{T}$, we create a propositional variable 
$p_{f(t)}$ that is true iff $f(t) \in \mathcal{S}$.
