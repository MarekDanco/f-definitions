...
\end{equation}
where $F$  and $Q$ are quantifier‑free formulas in the SMTLIB theory UFLIA of uninterpreted function symbols and linear integer arithmetic and $\forall x$ stands for universal quantification of arbitrarily many variables. We will define a way of certifying satisfiability of formulas in this class\todoSR{I am considering to generalize the certificate to the case of several universally quantified variables}\todoSR{We have no completeness result for such certificate}. Based on this, we will design an algorithm for detecting satisfiability under the following additional assumptions:
\begin{itemize}
...
%    \item for each $f_i$, the \emph{offset terms} $t^{f_i}_1,\dots,t^{f_i}_{l_i}$ are pairwise distinct LIA ground terms, and
    ...
    \item For each function symbol $f_i$, all its arguments in $Q$ have the \emph{same} coefficient of $x$ and this coefficient is non-negative.
\end{itemize}
...
Throughout this section we assume that $\phi$ is a formula of the form $F \land \forall x.\; Q$, as defined in ~\cref{sec:problem-statement}. Let $f_1, \dots, f_r$ be the uninterpreted function symbols appearing in $Q$. For each $f_i$, let $T_i = \args(Q, f_i) = \{t^{f_i}_1, \dots, t^{f_i}_{l_i}\}$ be its argument terms in $Q$.\todoSR{I am not sure about the argument terms where the coefficient of $x$ is zero.}
...
\begin{itemize}
    \item \textbf{Maximality condition $\Phi_{\text{max}}(\mathcal{S})$:}\todoSR{I think this could be weakened if the coefficient of $x$ is $2$ or more, but this would lead too far for our submission.}
    \[
...
% \begin{algorithm}
% \caption{Interval‑Based Satisfiability Check (for $\numvar{x}=1$)}
% \label{alg:interval-check-param-withBigvee}
% \begin{algorithmic}[1]
% \Require $\phi = F \land \forall x.\, Q$ with $\numvar{x}=1$, as defined in the implementation focus of Section~\ref{sec:problem-statement}
% \Ensure \textsc{Sat} or \textsc{Unsat}
% \State $b_{\min} \gets 0$, $b_{\max} \gets 0$ 
% \While{$F \land \bigwedge_{z=b_{\min}}^{b_{\max}} \subst{Q}{x}{\repr{z}}$ is satisfiable}
%     \State $\text{max}_\text{up}\gets\bigvee_{S_1\subseteq T_1,\dots,S_r\subseteq T_r}\Psi_{\text{up}}([b_{\min},b_{\max}],(S_1,\dots,S_r))$
%     \State $\text{min}_\text{down}\gets\bigvee_{S_1'\subseteq T_1,\dots,S_r'\subseteq T_r}\Psi_{\text{down}}([b_{\min},b_{\max}],(S_1',\dots,S_r'))$ 
%     \If{$\left(F \land \bigwedge_{z=b_{\min}}^{b_{\max}} \subst{Q}{x}{\repr{z}}\right) \land \text{max}_{\text{up}} \land \text{min}_{\text{down}}$ is satisfiable}
%         \State \Return \textsc{Sat} 
%     \EndIf
%     \State $b_{\min} \gets b_{\min} - 1$, $b_{\max} \gets b_{\max} + 1$ \Comment{Expand interval symmetrically}
% \EndWhile
