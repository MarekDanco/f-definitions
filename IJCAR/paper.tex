...
\title{Proving Satisfiability of\\Universally Quantified Integer Constraints with\\ Uninterpreted Function Symbols%
   \todoSN{Not yet sure for Mikoláš and Marek for affiliation.} \todoSN{Title options seen in comments by mj.}
    }
...
For any choice of subsets $\mathcal{S}\subseteq\mathcal{T}$,\todoSR{I think we could weaken the ``same coefficient condition'' by requiring it only for the $f$ occurring in $\mathcal{S}$. In a similar way, we could allow several quantified variables, and restrict the coefficients of quantified variables in $\mathcal{S}$.}\todoSN{Ah, yes, I think it's doable as functions not in $\mathcal{S}$ don't need propagation and only propagating functions need strict ordering!} define:
\begin{itemize}
