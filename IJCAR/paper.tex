...
\title{Certifying Satisfiability of\\Universally Quantified Integer Constraints with\\ Uninterpreted Function Symbols%
  \todoSN{Title options seen in comments by mj.}
    }
...
In this section, we will introduce an algorithm that will check satisfiability of an input formula by computing a satisfiability certificate of a certain form. \todoSN{Might need to change ``While..."}While our certificate framework (~\cref{def:certificate}) supports formulas with arbitrary numbers of universally quantified variables ($\numvar{\bar{x}} \geq 1$), the algorithm presented here focuses on the case where propagation can be performed along a \emph{single distinguished variable} among $\bar{x}$. This certificate will explicitly certify: (1) satisfiability of the quantifier-free part $F$, and (2) satisfiability over a finite interval $B = [b_{\min}, b_{\max}]$ of instantiations of the quantified part $Q$ w.r.t. the distinguished variable. For values outside $B$, it uses satisfiability propagators with an order that propagates upward from $b_{\max}+1$ and downward from $b_{\min}-1$.  We will show soundness of the algorithm and completeness relative to conditions that ensure that such propagation outward from a base interval $B$ is possible.

Throughout this section we assume that $\phi$ is a formula of the form $F \land \forall \bar{x}.\; Q$, as defined in ~\cref{sec:problem-statement}. Let $\mathcal{F}$ be the set of uninterpreted function symbols in $Q$, and $\mathcal{T}$ the set of terms of the form $f(t)$ in $Q$. 

\paragraph{Propagation variable.} We designate one variable $x_p\in\bar{x}$ as the \emph{propagation variable}\todoSN{What would happen is there is no such $x_p$?}. Let $\mathcal{T}_p\subseteq\mathcal{T}$ be subset of terms such that there is an uninterpreted function symbols $f$, variable $x_p\in\bar{x}$ and $c\in\mathbb{N}$ such that for all $f(t)\in\mathcal{T}$, the coefficient of $x_p$ in $t$ is $c$, and the coefficient of all other variables of $\bar{x}$ in $t$ is zero. In other words, each term in $\mathcal{T}_p$ has the form
$f(c \cdot x_p + d)$, where $d$ is a ground term.

\begin{example}
    Consider the formula \[g(c)=0\wedge \forall x\forall y\;.\; f(2x+c)= f(2x+c-1)+g(x+y).\]
    The set $\{ f(2x+c), f(2x+c-1)\}$ can be taken as $\mathcal{T}_p$ with propagation variable $x_p=x$. The term $g(x+y)$ cannot be in $\mathcal{T}_p$ since both quantified variables have non-zero coefficient.
\end{example}

% \paragraph{Propagation Terms.} For a given universally quantified variable $x_p \in \bar{x}$, we define $\mathcal{T}_p \subseteq \mathcal{T}$ as the set of terms $f(t) \in \mathcal{T}$ such that:
% \begin{itemize}
%     \item For all $f(t) \in \mathcal{T}$ with the same function symbol $f$, the coefficient of $x_p$ in $t$ is the same non-negative integer $c \in \mathbb{N}$.
%     \item For all $f(t) \in \mathcal{T}$ with the same function symbol $f$, the coefficient of all other variables $x_q \in \bar{x}$ with $q \neq p$ in $t$ is zero.
% \end{itemize}
% In other words, $\mathcal{T}_p$ contains those terms where $x_p$ is the only quantified variable that appears with a non-zero coefficient, and this coefficient is uniform across all terms for each function symbol.

% \begin{example}
% Consider the formula 
% \[g(c)=0\wedge \forall x\forall y\;.\; f(2x+c)= f(2x+c-1)+g(x+y).\]
% Here, $\mathcal{T}_1 = \{ f(2x+c), f(2x+c-1)\}$ corresponds to variable $x$, since both terms contain $x$ with coefficient 2 and $y$ with coefficient 0. The term $g(x+y)$ is not in any $\mathcal{T}_p$ since both quantified variables have non-zero coefficient.
% \end{example}

We will now formalize the condition ensuring propagation outward from a base interval $B$ is possible. For upward propagation, we select a $\mathcal{S} \subseteq\mathcal{T}_p$ whose argument terms are \emph{maximal} (all equal and strictly greater than other argument terms of the same function symbol). The \emph{propagability condition} then guarantees that, no matter what values are assigned to the non‑maximal occurrences, we can find values $v_1,\dots,v_r$ such that assigning $v_i$ to all occurrences in $\mathcal{S}$ makes $Q$ true. Different functions may receive different values. The downward condition is symmetric, selecting \emph{minimal} terms.

\begin{definition}[\req Condition]
\label{def:reqpivot-param}

For any choice of subsets $\mathcal{S}\subseteq\mathcal{T}_p$, define:
\begin{itemize}
    ...
    \[
        ...
        \bigwedge_{f(t) \in \mathcal{S}, f(t') \in \mathcal{T}_p\setminus\mathcal{S}} t > t'
        \right)}{x_p}{0}.
    \]
    
    \item \textbf{Upward propagability condition $\Phi_{\text{prop-up}}(\mathcal{S})$:}
        \[\forall \bar{y} \forall x_p \forall \bar{u} \exists \bar{v}.\ Q'(\mathcal{S})\]
        where $\bar{y}=\bar{x}\setminus\{x_p\}$, $\bar{u}$ corresponds to the variables $u_{f(t)}$ with $f(t)\in\mathcal{T}_p\setminus\mathcal{S}$ and $\bar{v}$ to the variables $v_{f(t)}$ with $f(t)\in \mathcal{S}$, and $Q'$ is obtained from $Q$ by simultaneously replacing
        \begin{itemize}
            \item every $f(t)\in\mathcal{T}_p\setminus\mathcal{S}$ by $u_{f(t)}$, and
            \item every $f(t)\in\mathcal{S}$ by $v_{f(t)}$.
        \end{itemize}
\end{itemize}
 
Symmetrically, for downward propagation with a subset $\mathcal{S}'\subseteq\mathcal{T}_p$, define $\Phi_{\text{min}}(\mathcal{S}')$ (with $<$ instead of $>$) and $\Phi_{\text{prop-down}}(\mathcal{S}')$.

We say that $Q$ satisfies the \emph{\req condition for $(\mathcal{S}, \mathcal{S}')$} (w.r.t. propagation variable $x_p$) if
\begin{itemize}
...
We say $Q$ satisfies the \emph{\req condition} (w.r.t. $x_p$) if there exists \emph{some} pair~$(\mathcal{S}, \mathcal{S}')$ such that $Q$ satisfies the \req condition for $(\mathcal{S}, \mathcal{S}')$.
\end{definition}

\begin{example}[\req Illustration]
\label{ex:req-illustration-multi}
Consider the formula:
\[
\forall x \forall y.\; f(2x + 3) + f(2x + 4) = g(x + y) + h(y)
\]
with propagation variable $x_p = x$. The coefficient of $x$ in the arguments of $f$ is $2$, and $g$, $h$ do not only depend on $x$, so we take
\[
\mathcal{T}_p = \{ f(2x + 3), f(2x + 4) \}.
\]

\medskip\noindent\textbf{Upward propagation:}
Consider $\mathcal{S} = \{ f(2x+4) \}$.
\begin{enumerate}
    \item \textbf{Maximality $\Phi_{\text{max}}(\mathcal{S})$:}
        For $f$: we require $(2x+4) > (2x + 3)$ when $x=0$, i.e., $4 > 3$, which holds.
        
    \item \textbf{Propagability $\Phi_{\text{prop-up}}(\mathcal{S})$:}
        Replace $f(2x+4)$ by $v$, $f(2x+3)$ by $u$, leaving $g(x+y)$ and $h(y)$ unchanged. The condition becomes
        \[
        \forall y \forall x \forall u \exists v.\; u + v = g(x+y) + h(y),
        \]
        which is valid: choose $v = g(x+y) + h(y) - u$.
\end{enumerate}

\medskip\noindent\textbf{Downward propagation:}
Take $\mathcal{S}' = \{ f(2x+3) \}$.
\begin{enumerate}
    \item \textbf{Minimality $\Phi_{\text{min}}(\mathcal{S}')$:}
        We require $(2x+3) < (2x+4)$ when $x=0$, i.e., $3 < 4$, which holds.
        
    \item \textbf{Propagability $\Phi_{\text{prop-down}}(\mathcal{S}')$:}
        Replace $f(2x+3)$ by $v'$, $f(2x+4)$ by $u'$, obtaining
        \[
        \forall y \forall x \forall u' \exists v'.\; v' + u' = g(x+y) + h(y),
        \]
        again valid by choosing $v' = g(x+y) + h(y) - u'$.
\end{enumerate}

Hence, the formula satisfies the \req condition with the chosen sets.
\end{example}
...
Now, given a concrete interval $B = [b_{\min}, b_{\max}]$ for $x_p$, we need additional ``clash conditions" that prevent the cells generated during propagation from clashing with the ground part $F$.
...
\]
is satisfiable, then $\phi$ has a satisfiability certificate, and hence is satisfiable.\todoSR{I moved the choice of $\mathcal{S}$ and $\mathcal{S}'$ into the formula, in the same way as in the algorithm}
\end{theorem}
