...
\testlength{
\begin{proof}
Assume $\phi$ is satisfiable and satisfies \req condition for $(\mathcal{S}^*,\mathcal{S}'^*)$. Let $\mathcal{M}$ be a model of $\phi$. Define an  interpretation $I^*$\todoSR{Why not work directly with $I$?}. For each function $f \in \mathcal{F}$, let $c_f$ be the common coefficient of $x$ in all its arguments. For each $f(t)\in\mathcal{F}$, define:
\[
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For each $f(t)\in\mathcal{S}^*$ and $a \in \args(F,f)$:\\ By construction of $b_{\max}$ and since $I^*(\subst{t}{x}{0})=v_{\max}^f$, we have:   % \[
    ...
    \end{itemize}
    Hence $I^* \models \Psi^+(B,\mathcal{S}^*)$.
    ...
    \begin{itemize}
        ...
        \[
        I^*(a)\cmpsign{\coeffsign{f}-}  c_f \cdot b_{\min} + v_{\min}^f = c_f \cdot b_{\min} + I^*(\subst{t}{x}{0}) = I^*(\subst{t}{x}{\repr{b}_{\min}}),
        \]
        which is the clash condition in $\Psi^-(B,\mathcal{S}'^*)$.
        
    \end{itemize}
    Thus $I^* \models \Psi^-(B,\mathcal{S}'^*)$.
\end{enumerate}
...
\]
Since $[-N, N]\supseteq B$ and $I^*$ satisfies all three conditions as shown above, this formula is also satisfiable (witnessed by $I^*$). Therefore, the algorithm returns \textsc{Sat}.
\qedhere\end{proof}}
