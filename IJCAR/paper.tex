...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item For $z > b_{\max}$: the condition $\Phi_{\text{prop}}(\mathcal{S})$ provides, for any given values for all non‑propagated cells in $\Gamma_{I,z}(\phi)$, values $v_{f(t)}$ for each $f(t)\in\mathcal{S}$ such that assigning these values to each corresponding cells in $X_z$ satisfies $Q[z]$.
