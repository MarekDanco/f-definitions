...
%     \end{itemize}
given by the following table, where the first column indicates the order $\preceq$:
    \[\small
    \begin{array}{|l|l|l|l|}\hline
      & X_{(x,y)} & propagators\\ \hline
        x=3, y=0  & \{g(0)\} & g(0)\mapsto 5\\
        x=3,y=1,2,\dots & \{f(3,y), g(y+1)\} & f(x,y)\mapsto y+g(y)-1, g(y+1)=g(y)-2\\
        x=3, y=-1,-2,\dots &  \{f(3,y), g(y)\} & f(x,y)\mapsto y+g(y)-1, g(y)=g(y+1)+2\\
...
\begin{enumerate}
    \item \textbf{Pre‑satisfiability certificate:} Define the cell interpretation~$I$ as follows:
      \begin{itemize}
        ...
        \item For every $a\in args(F, f)$,  where $f$ is an uninterpreted function symbol, $I(f(I^*(a)))=I^*(f)(I^{*}(a))$
    \end{itemize}
    By construction, $I$ is finite and satisfies $F$, because $I^*$ satisfies that formula and $I$ agrees with $I^*$ on all relevant cells.
  ...
  \item \textbf{Propagated cells $X_z$ for $z \in \mathbb{Z}$:}
    ...
    \begin{itemize}
    \item For $z \in B$: In this case, $\Gamma_{I,z}(\phi)\setminus \mathit{def}(I)\setminus X_z$ is empty. The satisfiability propagator simply sets the values of the cells in $X_z$ to the values given by $I^*$.
