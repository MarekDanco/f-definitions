...
Throughout this section we assume that $\phi$ is a formula of the form $F \land \forall \bar{x}.\; Q$, as defined in ~\cref{sec:problem-statement}. Let $\mathcal{F}$ be the set of uninterpreted function symbols in $Q$, and $\mathcal{T}$ the set of terms of the form $f(t)$ in $Q$. Let $\mathcal{T}_p$ be the set of elements of $\mathcal{T}$ such that there is an uninterpreted function symbols $f$, variable $x_p$ in $\bar{x}$ and $c\in\mathbb{N}$ such that for all $f(t)\in\mathcal{T}$, the coefficient of $x_p$ in $t$ is $c$, and the coefficient of all other variables of $\bar{x}$ in $t$ is zero.

For example for the formula \[g(c)=0\wedge \forall x\forall y\;.\; f(2x+c)= f(2x+c-1)+g(x+y),\] $\mathcal{T}_p$ is $\{ f(2x+c), f(2x+c-1)\}$. The term $g(x+y)$ is not in $\mathcal{T}_p$ since both quantified variables have non-zero coefficient.
...
\]
is satisfiable, then $\phi$ has a cell propagation certificate, and hence is satisfiable.\todoSR{I moved the choice of $\mathcal{S}$ and $\mathcal{S}'$ into the formula, in the same way as in the algorithm}
\end{theorem}
