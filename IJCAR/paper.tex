...
The algorithm is guaranteed to terminate under an additional semantic condition enabling induction (\cref{def:reqpivot-param}). We illustrate the class of formulas our algorithm can handle using two examples. 
\begin{example}
  \label{ex:formula}
...
Instead of enumerating all subset choices we introduce auxiliary Boolean variables that represent membership in the subsets $\mathcal{S}$ and $\mathcal{S}'$. For each term $f(t)\in\mathcal{T}$, we create:
\begin{itemize}
...
We prepared a test suite consisting of 29 problems\footnote{All the data for our experiments can be found at our public repository \\\url{https://github.com/MarekDanco/f-definitions}}
such as
