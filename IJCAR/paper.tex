...
The last conjunct in $\Psi_{\text{up}}(B,\mathcal{S})$\todoSN{Is this explanation needed?} ensures that during upward propagation, every 
non‑propagated cell's argument lies at or to the right of $b_{\min}$. This prevents upward propagation 
from depending on cells that would be assigned later by downward propagation, since in the well‑order 
of \cref{thm:interval-certificate-param} all $z > b_{\max}$ are processed before any $z < b_{\min}$. No symmetric constraint is needed for 
$\Psi_{\text{down}}$ because downward propagation occurs later and may use already‑assigned upward cells.
...
The clash condition $a \leq \subst{t}{x}{\repr{b}_{\max}}$ in $\Psi_{\text{up}}(B,\mathcal{S})$ guarantees that for every ground argument $a \in \args(F,f)$:
      ...
      $I(\subst{t'}{x}{\repr{z}}))\neq I(\subst{t}{x}{\repr{z}})$. 
      Since $t'$ and $t$ are arguments of the same function symbol, the respective coefficients of $x$ in $t$ and $t'$ are the same. Hence 
        \[
