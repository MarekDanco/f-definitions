...
For any choice of subsets $\mathcal{S}\subseteq\mathcal{T}$,\todoSR{I think we could weaken the ``same coefficient condition'' by requiring it only for the $f$ occurring in $\mathcal{S}$. In a similar way, we could allow several quantified variables, and restrict the coefficients of quantified variables in $\mathcal{S}$.} define:
\begin{itemize}
