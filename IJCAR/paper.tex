...
We prepared a test suite consisting of 28 problems such as\todoSR{CFP:``executables of systems, and input data of experiments should be made available'' e.g., via a reference to a website}
...
We implemented our algorithms in the language Python, using the SMT solver \textsc{z3}\todoSR{correct?} for all satisfiability checks. We also studied the capabilities of the \textsc{smt} solvers \textsc{z3} and
\textsc{cvc5}---with model based quantifier instantiation~\cite{ge-cav09} explicitly turned on---to directly solve the example.
