...
Hence $I(\subst{t'}{x}{\repr{z}'})< I(\subst{t}{x}{\repr{z}}))$, which implies that the two sides of the inequality cannot be the same.
  ...
  \item The case $z < b_{\min}$ is symmetric, using the downward clash condition $a \geq \subst{t}{x}{b_{\min}}$ and $\Phi_{\text{min}}(\mathcal{S}')$. Here, we prove that for all $z'\preceq z$, $I(\subst{t'}{x}{\repr{z}'})> I(\subst{t}{x}{\repr{z}}))$.
