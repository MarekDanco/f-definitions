...
The formula in the premise of \cref{thm:interval-certificate-param} belongs to Presburger arithmetic (i.e., SMTLIB LIA), and hence its satisfiability can be algorithmically checked.
\todoSR{a concrete checkable certificate would be $I, B, \mathcal{S}, \mathcal{S}'$}
Moreover, the theorem immediately yields an algorithm that searches for the parameters $B$, $\mathcal{S}$ and $\mathcal{S}$ of this theorem. This can be seen in \cref{alg:interval-check-param-withBigvee}.
...
\begin{itemize}
    ...
    \item Upward propagation encoding\todoSR{any idea for improving notation here, is welcome}:

        \[\bigwedge_{\sigma_{f(t)}\in\{\neg,\cdot\},f(t)\in\mathcal{T}} \big(\bigwedge_{f(t)\in\mathcal{T}} \sigma_{f(t)}s_{f(t)}\Rightarrow\forall x \forall \bar{u} \exists \bar{v}.\ Q'\big)\]
      ...
      \item Downward propagation encoding: symmetrical
