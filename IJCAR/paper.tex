...
%    \item for each $f_i$, the \emph{offset terms} $t^{f_i}_1,\dots,t^{f_i}_{l_i}$ are pairwise distinct LIA ground terms, and
    ...
    \item The set of coefficient vectors of the quantified variables in all argument terms of uninterpreted function symbols in $Q$ is collinear, and the non-zero coefficient vectors of the arguments of each individual uninterpreted function symbol are identical.
    \item An additional semantic condition enabling induction (\cref{def:reqpivot-param})
\end{itemize}

\begin{example}
  \[f(0)=0 \wedge \forall x,y \,.\, f(2x+2y+3)=g(x+y)+f(2x+2y+c+1),\]
  where $f$ has coefficient vector $(2,2)$ in all its arguments, $g$ has coefficient vector~$(1,1)$, and $c$ is an uninterpreted constant. Both function symbols satisfy the same‑coefficient requirement.
\end{example}
